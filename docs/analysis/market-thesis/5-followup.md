...
상한도 그대로(왕복 3, tool call 20회, 결과 누적 100,000자).
...
- 프롬프트 규칙: 툴 결과에 없는 사실·숫자 금지, 투자 조언 금지, 그리고 **"결과를 알고 쓰는
  ...
  본다" 형태를 쓰게 한다. 숫자 표기는 추론 프롬프트와 같은 `llm.NUMBER_STYLE`을 쓴다.
...
- **해설의 근거가 될 문서 소스** — 이 문서를 쓸 때는 `einfomax`·`cnbc`·`bbc_business`와
  ...
  **6단계로 뺐고(2026-08-21, 사용자 요청) 2026-08-22에 구현했다**
  ([6-analyst.md](6-analyst.md)) — 네이버 증권 리서치 여섯 카테고리가 `document`로 들어오고
  KIS 투자의견·목표주가가 `analyst_opinions` 툴로 열렸다. 해설이 리포트를 근거로 인용할 수
  있게 된 것은 운영 반영 뒤부터다.
