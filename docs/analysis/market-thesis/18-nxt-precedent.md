...
- **애프터마켓 해설.** `NARRATED_SLOTS`는 그대로다. 이 단계의 질문(다음날 장전이 애프터를
  보는가)과 별개이고, 되돌리기 쉬운 것만 골랐다(2.5절). **다만 그 결과로 장전이 읽는 슬롯
  셋 중 하나만 `outcomes`가 영영 빈 배열이다** — 그 자리를 [19-nxt-narration.md](19-nxt-narration.md)가
  설계했다(2026-08-31). 착수 게이트는 2.5절의 5영업일 관측이다.
