"""툴 호출 원장(13단계). **툴박스가 쥔 상태 중 하나를 클래스로 뗀 것이다.**

기록만 쌓고 **DB에는 쓰지 않는다** — 읽기 전용 툴 셋이라는 성격을 유지하고 저장 시점은
부르는 쪽(`thesis/store.py`)이 정한다. `ThesisToolbox`가 하나를 소유하고 다섯을 그대로
...
from modules.thesis.domain import ToolCallErrorKind, ToolCallRecord, ToolLimitExceeded
