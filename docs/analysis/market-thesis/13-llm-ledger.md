...
**비용은 그 전까지 LangSmith 트레이스에만 있었다.** 원장이 왕복·호출·문자는 세는데 그것이
얼마였는지를 안 세서, 슬롯별 추이를 보려면 사람이 트레이스를 하나씩 열어야 했다.

셋으로 나눈 이유는 **서로 다른 손잡이에 붙기 때문이다.**

| 칸 | 무엇이 움직이나 |
| --- | --- |
| `prompt_tokens` | 프롬프트 블록 크기와 `MAX_TOOL_ROUNDS`. 왕복마다 대화 전체가 재전송된다 |
| `completion_tokens` | 답변 길이 + 사고 길이 |
| `reasoning_tokens` | 한 왕복에 새로 들어온 툴 결과의 양. 대화에 안 남아 재전송도 캐시도 안 된다 |

한 칸으로 묶으면 어느 쪽이 늘었는지 못 가른다. 2026-08-27 실측에서 reasoning이 출력의
79%였고 그중 62%가 호출 하나에 몰려 있었다 — 그 호출 직전에 툴 결과 26,221 토큰이
한꺼번에 들어왔다.

**값은 그래프 밖 콜백이 누적한다**(`llm.token_usage`, `ThesisBuilder.usage`). `tool_rounds`가
그래프 최종 상태가 아니라 툴박스에서 오는 것과 같은 이유다 — **실패한 대화는 최종 상태를
못 받는다.** 그래서 죽은 대화에도 그때까지 쓴 만큼이 남는다.

**NULL과 0을 가른다.** NULL은 이 칸이 생기기 전 행("안 쟀다")이고, 모델을 한 번도 못 부르고
죽은 대화는 0("안 썼다")이다.

**캐시 적중은 안 센다.** `input_token_details.cache_read`가 같은 자리에 있지만 칸을 늘리지
않았다 — 지금 그 값으로 판단할 것은 "`x-grok-conv-id`가 먹고 있나" 하나이고, 그건 트레이스
하나로 답이 난다. 적중률이 슬롯·요일에 따라 갈리는 것이 관측되면 그때 칸을 더한다.
