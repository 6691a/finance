...
GET /api/causal/directions?from=&to=&target_code=                    # 대상별로 접은 방향성
...
## 8.8 문서 검색을 BM25로 (2026-08-31 사용자 요구)

> **2026-09-02: 검색 기능을 통째로 뺐다**(사용자 지시). BM25도 `ILIKE` 대체도 없다 —
> `q=` 파라미터, 문서 화면의 검색 상자, 대시보드의 검색 폼이 전부 사라졌다. 다음 판은
> **쪽 단위 색인**으로 새로 짓고(§8.8.6), 그 작업은 다른 태스크에서 한다. 아래 절들은
> 그때 같은 덫을 다시 밟지 않으려고 남긴 기록이다.

main이 `document_bm25`·`document_attachment_bm25` 색인 둘을 만들었다
([pdf-parsing-bm25.md](../pdf-parsing-bm25.md) §8). 화면의 `q=`가 `ILIKE`였던 자리를 그것으로
넘긴다 — `ILIKE`는 `삼성전자가`와 `삼성전자는`을 다른 낱말로 보고 관련도 순서도 못 만든다.

- **문자열 SQL을 손으로 쓰지 않는다.** `sqlalchemy-paradedb`(사용자 지정)가 `|||`·`pdb.score`·
  `pdb.snippet`을 SQLAlchemy 식으로 만들어 준다. 그 셋이 운영 ParadeDB 0.25.2에서 그대로
  도는 것을 붙이기 전에 확인했다.
- **검색이면 정렬이 바뀐다.** 관련도 순이다 — 날짜 순으로 두면 "가장 잘 맞는 문서"를 묻는
  화면이 "가장 최근에 그 낱말이 나온 문서"를 준다. 그 둘은 다른 질문이라 캡션이 밝힌다.
- **첨부까지 함께 본다.** 본문이 첨부에만 있는 출처가 있어(한국은행·BOJ) 문서 색인만 보면
  그쪽 리포트가 검색에서 통째로 빠진다.

### 8.8.1 밟은 덫 셋 (전부 운영 실측)

- **OR 한 줄로는 못 쓴다.** `title ||| q OR id IN (첨부 조회)`는 ParadeDB가
  `Unsupported query shape`로 거절한다. 설계 §8.2가 말한 "둘을 합친 뒤 `document_id`로
  접는다"가 UNION ALL이라는 뜻이었다. 접는 이유는 중복이다 — 첨부가 넷인 문서(한국은행)가
  네 번 나오면 안 된다.
- **점수와 순위는 같은 조회에서 나와야 한다.** 순위를 A 조회로, 화면의 숫자를 B 조회로
  만들었더니 `5.51 · 5.51 · 6.36` 순으로 나왔다 — `pdb.score`는 **그 조회의** 점수라
  조회가 둘이면 값도 둘이다.
- **발췌는 그 쪽에 남을 행에만 만든다.** 순위 조회에 넣으면 매칭 전체에 계산되어 100건
  요청이 **5.5초**였다. id 배열로 따로 부르면 **0.5초**다. 그래서 검색일 때 쪽 크기도
  100에서 30으로 줄였다 — 발췌는 본문을 다시 읽는 일이고, 관련도 순이라 뒤쪽은 볼 일도 적다.

### 8.8.3 한글이 깨지던 자리 (2026-09-01 사용자 보고)

검색창에 `기준`을 치면 `ㄱㅣㅈㅜㄴ`이 됐다.

**원인은 조합 중 다시 그리기다.** 한글 입력기는 낱자를 모아 글자를 만드는 조합 상태를
갖는데(`ㄱ` → `기` → `기ㅈ` → `기준`), 그 사이 React가 input의 `value`를 **확정된 값**으로
되돌리면 입력기가 조합을 이어 갈 자리를 잃고 낱자를 그대로 흘린다. 화면이 키 하나마다
`setParams`를 불러 라우터를 움직였으니 매 낱자마다 그 일이 일어났다.

곁가지가 둘 더 있었다 — **낱자마다 BM25 검색을 한 번씩** 쳤고(`기`·`기ㅈ`·`기주`…), 뒤로 가기
기록이 글자 수만큼 쌓였다.

고침은 `TextFilter` 하나다. 초안은 그 컴포넌트가 들고 **Enter나 포커스를 잃을 때만** 밖으로
넘긴다. **조합 중의 Enter는 무시한다** — 그 Enter는 "검색해라"가 아니라 "이 글자로
확정해라"다. 밖에서 값이 바뀌면(뒤로 가기, 링크) 초안이 따라간다.

같은 규칙을 글자 필터 전부에 쓴다(출처·최소 점수·종목 태그·종목코드). 값이 그대로면 내보내지
않아서, 훑고 지나가는 포커스가 요청을 만들지 않는다.

### 8.8.2 하이라이트는 HTML이 아니라 마커다

서버가 `[[hl]]…[[/hl]]`로 감싸고 화면이 그것을 쪼개 `<mark>`를 만든다. `<b>`를 그대로 주면
화면이 `dangerouslySetInnerHTML`을 써야 하는데, **검색어가 곧 사용자 입력이라** 그 입력이
발췌에 실려 되돌아온다. 그 자리를 열지 않는다 — 테스트가 `<img onerror=…>`를 넣어 태그로
살아나지 않는 것을 본다.

대시보드에 검색 상자를 뒀다. **결과는 문서 화면이 그린다** — 대시보드에 표를 또 만들면
필터와 쪽 이동이 한쪽에만 붙는다.

### 8.8.4 왜 되돌렸나 — mutable 세그먼트가 조회 시점에 토크나이즈된다 (2026-09-01)

검색이 안 끝났다. 화면이 다시 부를 때마다 백엔드가 하나씩 늘어 **25개가 25분씩 운영 DB의
CPU를 먹었고**, 그 사이 시세·지표 화면까지 함께 느려졌다. 검색은 `ILIKE`로 되돌린 상태다.

**원인은 `document_attachment_bm25`의 mutable 세그먼트다.** pg_search는 갓 쓴 행을 굳지 않은
mutable 세그먼트에 두는데, **조회할 때마다 그 안의 행을 그 자리에서 토크나이즈한다.** 첨부
본문은 PDF에서 뽑은 글이라 크다(924건 · 평균 20,372자 · p90 50,910자 · 최대 307,511자).
그래서 조회 한 번이 수십만~수백만 자를 토크나이즈하는 일이 된다.

셋을 나란히 재서 확인했다(같은 색인, 같은 검색어).

| mutable 세그먼트 | 첨부 조회 |
| --- | --- |
| 806건 | **안 끝남** |
| 50건(120만 자) | **155초** — 그 50건을 직접 토크나이즈하니 **142초**였다 |
| 2건 | **0.14초** |

`document_bm25`가 멀쩡했던 이유도 같다 — 본문이 작아서 mutable에 무엇이 있든 싸다.

**갈라 죽인 가설들**(전부 운영 실측).

- 점수 계산이 비싼 것이 아니다 — `pdb.score`를 빼도 같다.
- UNION이 문제가 아니다 — 첨부 하나만 봐도 같다.
- 31건을 채우려 반복하는 것이 아니다 — 매칭 **0건**인 검색어도 똑같이 안 끝난다.
- 삭제 표시(`num_deleted` 630/637)가 원인이 아니다 — `REINDEX`로 0으로 만든 뒤에도 155초였다.
  `REINDEX`가 도운 것은 **mutable을 비운 것**이지 시체를 치운 것이 아니다.

**취소가 안 먹는다.** `statement_timeout`을 5초·20초·150초로 걸어도 죽는 시점은 그 토크나이즈
한 판이 끝난 뒤다(81~165초). `pg_cancel_backend`가 `true`를 돌려주고도 안 죽은 것이 같은
이유다. **그래서 `apps/core/database.py`의 `statement_timeout`은 이 사고를 못 막는다.**

### 8.8.5 되살리려면 무엇을 바꿔야 하나

지금 이 순간은 다섯 형태가 다 1.4초 안에 끝난다(API가 쓰던 UNION 형태 **1.13초** · 31건).
mutable이 2건이라서다. **새 PDF가 파싱되면 다시 쌓인다** — `mutable_segment_rows = 50`이면
최악이 약 140초다. 그래서 그대로 켜면 같은 사고가 다시 난다.

- **`mutable_segment_rows`를 더 낮춘다.** 5면 최악이 1/10이다. 대신 작은 immutable 세그먼트가
  자주 생긴다.
- **색인 대상을 앞 N자로 자른다.** 비용이 글자 수에 제곱으로 붙어서(같은 문서 8,000자 0.020초,
  16,000자 0.109초, 307,511자 260초 초과) 자르는 효과가 크다. 잃는 것은 적다 —
  `기준금리`로 재면 앞 16,000자에 **221/230**, 앞 8,000자에 **207/230**이 남는다.
  색인이 `((left(extracted_text, 16000))::pdb.lindera('korean'))`가 되고 조회도 같은 식을 쓴다.
- **화면은 재시도를 쌓지 않는다.** 위 둘을 해도 한 번의 느린 조회가 스물다섯 개가 되면 안 된다.

### 8.8.6 다음 판은 쪽 단위 색인이다 (2026-09-02)

**한 문서 = 한 색인 문서가 문제의 뿌리다.** 첨부 하나가 최대 307,511자라 조회 비용도,
BM25 점수도, LLM에 넘길 크기도 전부 그 크기에 묶인다. 쪽으로 나누면 셋이 함께 풀린다.

파서가 이미 `<!-- page:N -->` 표식을 넣고 있어 나눌 데이터가 있다. 쪽 단위 실측:

| | 문서 `body`(기사) | 첨부 통째로 | 첨부의 한 쪽 |
| --- | --- | --- | --- |
| 건수 | 2,886 | 924 | **12,140** |
| 중앙값 | 1,143자 | 7,339자 | **1,252자** |
| p90 | 4,292자 | 50,910자 | **3,186자** |
| 최대 | 37,715자 | 307,511자 | **14,107자** |

쪽으로 나누면 첨부가 **기사와 같은 크기 등급**이 된다. 8,000자를 넘는 쪽은 12,140개 중
49개(0.4%)이고 16,000자를 넘는 쪽은 없다 — 토크나이즈 비용 곡선의 평평한 구간이다.

- **mutable 세그먼트 5건 비용**: 지금 최악 260초 → **0.09초**.
- **커버리지**: 앞 N자로 자르는 안(96%)과 달리 **100%**.
- **순위**: BM25는 문서 길이로 점수를 깎는다(fieldnorm). 40쪽 리포트에 한 번 나온 낱말은
  짧은 기사보다 늘 밀린다. 쪽 단위면 그 왜곡이 사라진다.
- **LLM**: 30만 자(약 15만 토큰)를 통째로 줄 수 없다. 맞은 쪽 서너 장(장당 약 800 토큰)이
  실제로 가능한 유일한 단위이고, **그 단위가 곧 검색 단위여야 한다.** `page:N`이 근거
  표시가 된다.

손볼 곳은 다섯이다. ① `document_attachment_page` 모델과 마이그레이션(BM25 색인 포함),
② 파싱 DAG이 쪽 행을 함께 쓴다(`AttachmentPdfParser.parse()`가 이미 `ParsedPage` 목록을
갖고 있다), ③ 기존 924건은 `extracted_text`를 표식으로 쪼개는 backfill로 끝난다(재파싱
불필요), ④ API가 첨부 가지를 쪽 테이블로 바꾸고 발췌를 쪽에서 만든다, ⑤ 운영에서
`mutable_segment_rows`를 기본값으로 되돌린다. ①~③은 main 쪽 일이고 ④는 이 워크트리다.

## 8.7 주간 방향성과 종목 마스터의 뜻 변화 (2026-08-31)

main이 17단계(그래프 조회)를 구현하면서 표 하나가 늘고, 종목 마스터의 뜻이 바뀌었다.

- **`market_causal_direction`** — 한 주의 경로를 대상 하나로 접은 방향성. 추론이 관측 상태로
  읽는 값이라 화면에도 있어야 한다. `/api/causal/directions`와 인과 화면의 `방향성`
  데이터셋이 그 자리다.
  - **`bias`는 LLM이 정하고 세기는 코드가 센다.** 그래서 `up_count`가 더 많은데 `mixed`인
    행이 정상이다(실측: KOSPI 08-17 주가 위로 4·아래로 2인데 종합은 `엇갈림`). 화면이 둘을
    나란히 보이는 이유가 그것이다 — 하나만 보이면 다른 하나를 유도된 값으로 읽는다.
  - **채널별 세기도 함께 낸다.** 종합을 못 믿을 때 보는 재료라 접기 전 숫자를 감추지 않는다.
  - **예측이 아니라 사전 맥락이다.** 주 `W`를 `W+2` 월요일에 접으므로 추론이 보는 것은 최소
    9일 전 인과다 — 화면의 note와 대시보드 카드가 그 나이를 밝힌다.
- **`instrument`의 행이 있다는 것과 `is_watched`는 다른 뜻이 됐다.** 행이 있으면 문서에서 그
  종목을 알아보고 리서치를 받고, `is_watched`가 참이어야 시세를 받는다(실측 20종목 중 2종목).
  수집 화면의 열 이름을 `추적`에서 **`시세 수집`**으로 바꾸고 응답 설명도 고쳤다 — 화면이
  "추적"이라고 말하면 나머지 18종목이 아무 것도 안 하는 것처럼 읽힌다.
