...
import { causalElements, channelId, edgeIds, targetId, walk } from "./causal";
...
describe("walk", () => {
  ...
  it("채널이 없으면 사건이 대상에 바로 닿는다", () => {
    expect(walk(row({ channels: [] }))).toEqual(["event", targetId(row())]);
  });
});
...
describe("causalElements", () => {
  ...
  it("노드마다 종류가 붙는다", () => {
    ...
    expect(classes).toEqual(["event center", "channel", "channel", "target"]);
  });
