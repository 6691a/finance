"""장후 리뷰 DAG와 `modules/thesis/review.py`.

채점·해설의 알맹이는 `modules/thesis/outcomes.py`·`thesis/store.py`에 있고
...
def test_the_as_of_time_is_the_close_not_the_run_time():
    ...
    """
    assert review.as_of(date(2026, 8, 21)) == datetime(2026, 8, 21, 6, 30, tzinfo=UTC)


def test_the_macro_window_starts_at_the_open():
    """장후의 창은 당일 09:00부터다. 장전(전 개장일 마감부터)과 다르다."""
    assert review.macro_window_start(date(2026, 8, 21)) == datetime(2026, 8, 21, 0, 0, tzinfo=UTC)
...
def test_the_index_guard_asks_for_the_close_bar_not_the_run_time():
    ...
    review.PostCloseReview(connection, run_date=date(2026, 8, 21)).check_ready(["005930"])

    statement, parameters = connection.cursor().calls[-1]
    assert "index_bar" in statement
    assert parameters[0] == review.as_of(date(2026, 8, 21))
    assert parameters[1] == review.GUARD_INDEX_SYMBOLS
...
def test_run_hands_build_and_store_every_argument_it_requires(monkeypatch):
    ...
    monkeypatch.setattr(
        review.PostCloseReview, "check_ready", lambda self, watched: guarded.append(list(watched))
    )
    monkeypatch.setattr(store, "ThesisStore", FakeStore)
    monkeypatch.setattr(common.ThesisRun, "build_and_store", fake_build_and_store)

    post_close = review.PostCloseReview(FakeConnection([]), run_date=run_date)
    written = post_close.run(dag_run_id="manual__1", try_number=1)

    assert written == 4
    # 필수 인자가 빠지면 여기서 `TypeError`다.
    signature.bind(post_close._run, **received)
    assert received["run_slot"].value == "post_close"
    assert received["macro_window_start"] == review.macro_window_start(run_date)
    # **확정 종가 guard는 종목만 본다.** 지수는 확정 종가 테이블에 없다.
    assert guarded == [["005930"]]
    # 리뷰는 해석이라 과거 예측 성적을 싣지 않는다. NXT 리뷰와 같다.
    assert received["past"] == {}
    assert post_close._run.as_of_at == review.as_of(run_date)
...
def test_the_morning_slot_still_divides_by_the_previous_close():
    ...
    result = review._horizon_return(store, pending(RunSlot.PRE_OPEN, None), date(2026, 8, 26))
...
def test_an_intraday_slot_divides_by_the_price_it_actually_saw():
    ...
    assert kwargs["target_bar_at"] == common.close_at(date(2026, 8, 26))
...
def test_an_intraday_thesis_without_a_base_price_stays_ungraded():
    ...
    assert review._horizon_return(store, pending(RunSlot.PRE_CLOSE, None), date(2026, 8, 26)) is None
    assert store.calls == []
