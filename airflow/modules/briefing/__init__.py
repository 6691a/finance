...
파트마다 파일 하나다. `market.py`, `documents.py`, `ops.py`가 각각 조회와
`render_blocks`, `render_text`를 갖는다. 시장 리포트의 조회는 연결과 기준 시각을 쥔
`MarketBriefingReader` 클래스이고, 나머지 둘은 아직 `collect_summary` 함수다. LLM은 문서 리포트만 쓴다 —
