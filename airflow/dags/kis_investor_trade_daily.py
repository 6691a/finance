...
**당일 종가는 잠정값이라 대조에서 뺀다.** 18:10에 받는 당일 종가는 KRX 종가가 아니라 NXT
애프터마켓(20:00까지)의 실시간가다(2026-09-18 실측). 그 값이 저장돼 있다가 다음 날 KRX 종가로
오면 어긋나 소급 조정으로 오탐되고, 전 구간을 재수집한 뒤 확인 재조회에서 또 당일 값이
움직여 죽는다(2026-09-15부터 매일 재수집, 09-17·18 실패). 그래서 **자기 거래일 당일에 쓴
행은 비교하지 않는다**(`select_close_conflicts.sql`). 다음 날 upsert가 확정값으로 덮는다.
그 사이 이 종가는 KRX 종가가 아니다 — 당일 종가를 읽는 쪽은 그 점을 알아야 한다.
