"""코스피 일일 전망 — 전망 한 건, LLM 실행 원장, 툴 호출.

**관계와 메모는 여기 없다.** 그쪽 원본은 Neo4j다(사용자 결정, 2026-09-02). 이 트리에 관계
테이블을 만들면 원본이 둘이 되고 어느 쪽이 맞는지 정해야 한다.

vocabulary는 Airflow의 `modules/kospi/domain.py`에 값이 한 벌 더 있다. Airflow는 `apps/`를
보지 못해 import하지 못하므로 중복을 허용하고 `tests/models/test_analysis_models.py`가 대조한다.

설계는 `docs/analysis/kospi-forecast.md`다.
"""

from datetime import date, datetime
from decimal import Decimal
from enum import StrEnum

from sqlalchemy import (
    BigInteger,
    Boolean,
    CheckConstraint,
    DateTime,
    ForeignKey,
    Integer,
    Numeric,
    Text,
    UniqueConstraint,
)
from sqlalchemy.dialects.postgresql import JSONB
from sqlalchemy.orm import Mapped, mapped_column

from apps.core.database import EntityBase, table_options
from apps.models.analysis._columns import _enum_column


class KospiSlot(StrEnum):
    """전망을 만든 슬롯. **슬롯이 곧 기준가의 뜻을 정한다.**

    `pre_open`은 전일 종가 대비 오늘 종가를 묻고, `midday`·`pre_close`는 **그 시각 현재가
    대비** 오늘 종가를 묻는다. 정답은 셋이 같고 분모만 다르다.

    이름을 시각이 아니라 뜻으로 짓는다. 슬롯 시각은 운영 손잡이여서 30분을 옮기는 순간
    `slot_1135` 같은 이름은 거짓이 된다.
    """

    PRE_OPEN = "pre_open"
    MIDDAY = "midday"
    PRE_CLOSE = "pre_close"


class KospiDirection(StrEnum):
    """전망의 방향. **`flat`이 없다.**

    옛 추론의 3-클래스 확률이 캘리브레이션 실패의 자리였다(`prob_flat` 평균 0.31, 실제 13%).
    "얼마나 움직이나"는 기대 등락률과 폭이 이미 말한다.
    """

    UP = "up"
    DOWN = "down"


class KospiLlmRunKind(StrEnum):
    """대화의 종류. 전망과 관찰은 프롬프트도 판도 다르다."""

    FORECAST = "forecast"
    REVIEW = "review"


class KospiLlmRunStatus(StrEnum):
    """대화의 상태. `running`으로 열고 어떻게 끝나든 닫는다."""

    RUNNING = "running"
    SUCCEEDED = "succeeded"
    FAILED = "failed"


class KospiToolCallErrorKind(StrEnum):
    """툴 호출이 실패한 종류. **어디서 막혔는지가 다르다.**"""

    UNKNOWN_TOOL = "unknown_tool"
    VALIDATION = "validation"
    LIMIT = "limit"
    EXECUTION = "execution"
    CANCELLED = "cancelled"


class KospiForecast(EntityBase):
    """전망 하나. 슬롯마다 한 행이다.

    **첫 성공본은 불변이다.** 같은 `(run_date, slot)`에 행이 있으면
    `INSERT ... ON CONFLICT DO NOTHING`으로 아무 것도 바꾸지 않는다. LLM은 재호출마다 답이
    달라서 덮어쓰면 최초 판단이 사라진다. 잘못된 전망도 고치지 않는다 — 승인·보류 상태
    머신도, 사람이 행을 UPDATE하는 경로도 없다.

    **채점 칸 넷만 나중에 채워진다.** 그것도 한 번뿐이다(`graded_at IS NULL`이 조건이다).
    """

    __tablename__ = "kospi_forecast"
    __table_args__ = (
        UniqueConstraint("run_date", "slot", name="uq_kospi_forecast_natural_key"),
        CheckConstraint("slot IN ('pre_open', 'midday', 'pre_close')", name="ck_kospi_forecast_slot"),
        CheckConstraint("direction IN ('up', 'down')", name="ck_kospi_forecast_direction"),
        # 폭주만 받는 안전망이다. "방향과 부호가 맞아야 한다" 같은 정합성은 프롬프트와 저장
        # 전 검증이 본다 — DB로 막으면 모델이 경계값을 낼 때 행 전체가 사라진다.
        CheckConstraint("expected_change_pct BETWEEN -10 AND 10", name="ck_kospi_forecast_change_range"),
        CheckConstraint("band_pct BETWEEN 0.1 AND 5", name="ck_kospi_forecast_band_range"),
        CheckConstraint("base_price > 0", name="ck_kospi_forecast_base_price_positive"),
        CheckConstraint("rejected_reasons >= 0", name="ck_kospi_forecast_rejected_reasons"),
        # 채점 넷은 함께 있거나 함께 없다. 셋만 채워진 행은 읽을 수 없다.
        CheckConstraint(
            "(graded_at IS NULL AND actual_change_pct IS NULL AND hit IS NULL AND within_band IS NULL)"
            " OR (graded_at IS NOT NULL AND actual_change_pct IS NOT NULL"
            " AND hit IS NOT NULL AND within_band IS NOT NULL)",
            name="ck_kospi_forecast_grade_all_or_none",
        ),
        # 장중 슬롯만 "지금까지"가 있다. 장전에는 그 개념이 없다 — 아직 안 열렸다.
        CheckConstraint(
            "(slot = 'pre_open' AND so_far_pct IS NULL) OR slot <> 'pre_open'",
            name="ck_kospi_forecast_so_far_shape",
        ),
        table_options(
            comment="슬롯마다 만든 코스피 일일 전망을 불변으로 보존하는 테이블. 관계와 메모는 Neo4j가 갖는다",
            database="default",
        ),
    )

    run_date: Mapped[date] = mapped_column(
        nullable=False,
        comment="전망이 대상으로 삼은 세션 날짜(KST). 시각은 담지 않는다",
    )
    slot: Mapped[KospiSlot] = mapped_column(
        _enum_column(KospiSlot),
        nullable=False,
        comment=(
            "전망을 만든 슬롯(pre_open은 장전 08:35, midday는 장중 11:35, pre_close는 마감전 14:35 KST). "
            "슬롯이 기준가의 뜻을 정한다 — 장전은 전일 종가 대비, 장중 둘은 그 시각 현재가 대비다"
        ),
    )
    as_of_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        comment=(
            "관측 상태와 툴 조회의 기준 시각(UTC). 벽시계가 아니라 슬롯이 정한다. "
            "event-time cutoff라 이 시각 이후 감지·평가·갱신된 행은 조회에서 뺀다"
        ),
    )
    base_price: Mapped[Decimal] = mapped_column(
        Numeric(18, 4),
        nullable=False,
        comment="등락률의 분모. 장전은 직전 거래일 확정 종가, 장중은 그 시각 최신 분봉 종가다",
    )
    base_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        comment="기준가가 만들어진 시각(UTC). 장전은 직전 거래일 15:30 KST, 장중은 그 분봉의 시각이다",
    )
    so_far_pct: Mapped[Decimal | None] = mapped_column(
        Numeric(8, 2),
        nullable=True,
        comment=(
            "장중 슬롯만. 전일 종가 대비 현재가 등락률(퍼센트) — '지금까지 얼마나 왔나'다. "
            "예측 축(기준가 대비 마감까지)과 다른 값이라 칸을 나눈다. 장전은 NULL"
        ),
    )
    direction: Mapped[KospiDirection] = mapped_column(
        _enum_column(KospiDirection),
        nullable=False,
        comment="전망 방향(up/down). flat이 없다 — 크기와 폭이 '얼마나 움직이나'를 이미 말한다",
    )
    expected_change_pct: Mapped[Decimal] = mapped_column(
        Numeric(8, 2),
        nullable=False,
        comment="기준가 대비 기대 등락률(퍼센트). 부호가 있고 direction과 맞는다. 모델이 낸 값을 보정하지 않는다",
    )
    band_pct: Mapped[Decimal] = mapped_column(
        Numeric(8, 2),
        nullable=False,
        comment="기대 등락률의 ± 폭(퍼센트포인트). 상한이 아니라 폭이라 구간은 기대 ± 이 값이다",
    )
    reasons: Mapped[list] = mapped_column(
        JSONB,
        nullable=False,
        comment=(
            "검증을 통과한 이유 목록. 항목은 {direction, statement, factor, memory_id, slot_ref}이고 "
            "**순서가 곧 중요도다**(프롬프트가 결론에 크게 작용한 것부터 요구한다). 개수 상한은 없다"
        ),
    )
    weak: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        server_default="false",
        comment="검증을 통과한 이유가 0건인 답. Slack에 머리표가 붙는다 — 정상 답과 같아 보이면 안 된다",
    )
    rejected_reasons: Mapped[int] = mapped_column(
        Integer,
        nullable=False,
        server_default="0",
        comment="검증이 버린 이유 수. 남은 수(reasons 길이)의 분모라 근거 유효율이 여기서 읽힌다",
    )
    input_state: Mapped[dict] = mapped_column(
        JSONB,
        nullable=False,
        comment=(
            "모델이 본 관측 상태 전부(일봉·관계 표·메모·앞 슬롯). 관계와 메모는 Neo4j가 원본이라 "
            "다음 날 바뀐다 — 이 칸이 없으면 '그 전망이 무엇을 보고 나왔나'를 되짚을 수 없다"
        ),
    )
    actual_change_pct: Mapped[Decimal | None] = mapped_column(
        Numeric(8, 2),
        nullable=True,
        comment="장후 채점. 기준가 대비 오늘 KRX 확정 종가의 등락률(퍼센트). 종가가 없으면 NULL로 둔다",
    )
    hit: Mapped[bool | None] = mapped_column(
        Boolean,
        nullable=True,
        comment="방향 적중 여부. 실현 등락이 정확히 0이면 false다 — 어느 방향도 아니면 맞은 것이 아니다",
    )
    within_band: Mapped[bool | None] = mapped_column(
        Boolean,
        nullable=True,
        comment="크기 적중 여부. abs(실제 - 기대) <= 폭. 방향이 틀려도 잰다 — 둘은 다른 축이다",
    )
    graded_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        comment="채점 시각(UTC). NULL이면 아직 안 했다는 뜻이고, 값이 있으면 다시 채점하지 않는다",
    )
    prompt_version: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="이 전망을 만든 프롬프트 판. 채점을 판별로 가르는 축이다",
    )
    llm_model: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="이 전망을 만든 모델 이름. 판과 함께 성적을 가르는 축이다",
    )
    dag_run_id: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="이 행을 쓴 Airflow dag_run_id. 같은 실행의 재시도인지를 DB가 증명한다",
    )
    llm_run_id: Mapped[int | None] = mapped_column(
        BigInteger,
        ForeignKey("kospi_llm_run.id", ondelete="SET NULL"),
        nullable=True,
        comment="이 전망을 만든 대화. 원장이 지워져도 전망 행은 남는다",
    )


class KospiLlmRun(EntityBase):
    """모델 호출 하나가 행 하나다. **실패해도 남는다.**

    모델을 부르기 전에 `running`으로 열고 어떻게 끝나든 닫는다. 실패한 대화가 원장에 없으면
    "안 돌았다"와 "돌다 죽었다"를 못 가르고, 패턴 분석이 성공한 실행만 보게 된다.
    """

    __tablename__ = "kospi_llm_run"
    __table_args__ = (
        CheckConstraint("kind IN ('forecast', 'review')", name="ck_kospi_llm_run_kind"),
        CheckConstraint(
            "status IN ('running', 'succeeded', 'failed')",
            name="ck_kospi_llm_run_status",
        ),
        CheckConstraint(
            "slot IS NULL OR slot IN ('pre_open', 'midday', 'pre_close')",
            name="ck_kospi_llm_run_slot",
        ),
        # 장후 관찰에는 슬롯이 없고 전망에는 반드시 있다. 종류가 그것을 정한다.
        CheckConstraint(
            "(kind = 'review' AND slot IS NULL) OR (kind = 'forecast' AND slot IS NOT NULL)",
            name="ck_kospi_llm_run_slot_shape",
        ),
        table_options(
            comment="코스피 전망·관찰의 모델 호출 원장. 실패한 대화도 남는다",
            database="default",
        ),
    )

    kind: Mapped[KospiLlmRunKind] = mapped_column(
        _enum_column(KospiLlmRunKind),
        nullable=False,
        comment="대화의 종류(forecast는 전망, review는 장후 관찰). 프롬프트도 판도 다르다",
    )
    run_date: Mapped[date] = mapped_column(
        nullable=False,
        comment="이 대화가 대상으로 삼은 세션 날짜(KST)",
    )
    slot: Mapped[KospiSlot | None] = mapped_column(
        _enum_column(KospiSlot),
        nullable=True,
        comment="전망 대화의 슬롯. 장후 관찰은 슬롯이 없어 NULL이다",
    )
    as_of_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        comment="이 대화의 조회 기준 시각(UTC). 슬롯이 정한다",
    )
    status: Mapped[KospiLlmRunStatus] = mapped_column(
        _enum_column(KospiLlmRunStatus),
        nullable=False,
        comment="대화의 상태. running으로 열고 succeeded 또는 failed로 닫는다",
    )
    llm_model: Mapped[str] = mapped_column(Text, nullable=False, comment="부른 모델 이름")
    prompt_version: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="이 대화가 쓴 프롬프트 판. 판을 안 올리고 문장을 고치면 이 원장이 거짓말을 한다",
    )
    dag_run_id: Mapped[str] = mapped_column(
        Text, nullable=False, comment="이 대화를 연 Airflow dag_run_id"
    )
    try_number: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="그 태스크의 시도 번호. 재시도가 대화를 몇 번 열었는지 본다"
    )
    started_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, comment="대화를 연 시각(UTC)"
    )
    finished_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        comment="대화를 닫은 시각(UTC). NULL이면 닫지 못하고 죽은 것이다",
    )
    error: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="실패 사유. 종류와 메시지를 함께 남긴다"
    )
    tool_rounds: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="조사 왕복 수. 상한에 붙어 있으면 조사가 잘리고 있다는 신호다"
    )
    tool_calls: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="기록된 툴 호출 수. 모르는 툴과 인자 검증 실패도 세므로 예산 카운터와 다른 수다",
    )
    tool_result_chars: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="모델에게 실제로 돌아간 툴 결과의 문자 수. 버려진 결과는 안 센다",
    )
    truncated: Mapped[bool | None] = mapped_column(
        Boolean,
        nullable=True,
        comment="조사가 왕복 상한에서 끊겼나. 스스로 끝낸 실행과 잘린 실행을 가르는 유일한 칸이다",
    )
    rejected: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="검증이 버린 이유·관찰의 수. 남은 수의 분모라 유효율이 여기서 읽힌다",
    )
    memories_written: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="이 관찰이 새로 쓴 메모 수. 전망 대화는 NULL"
    )
    memories_rejected: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="중복 또는 상한으로 쓰지 않은 메모 수"
    )
    memories_kept: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="모델이 keep으로 판정해 유지한 메모 수"
    )
    memories_dropped: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="모델이 drop으로 판정해 내린 메모 수"
    )
    memories_unreviewed: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="답에서 빠진 메모 수. keep으로 치지 않는다 — 두 번 연속이면 코드가 내린다",
    )
    memories_expired: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="나이 상한으로 내린 메모 수. 모델 판정과 무관하게 코드가 정한다",
    )
    prompt_tokens: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="입력 토큰. 왕복마다 대화 전체가 재전송되므로 프롬프트 크기와 왕복 수가 여기 실린다",
    )
    cached_tokens: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="입력 중 캐시에서 읽은 토큰. 제공처가 안 알려 주면 0이다. 최적화 효과를 재는 칸이다",
    )
    completion_tokens: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="출력 토큰. reasoning을 포함한다"
    )
    reasoning_tokens: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="출력 중 사고 토큰. 대화에 남지 않아 재전송되지도 캐시되지도 않는다",
    )


class KospiToolCall(EntityBase):
    """그 대화 안의 툴 호출 하나.

    **검증 전 인자와 검증 후 인자를 둘 다 남긴다.** 모델이 보낸 그대로가 `arguments`이고
    함수에 실제로 들어간 것이 `validated_arguments`다. 뒤가 NULL이면 함수에 닿지도 못했다.

    **결과 전문을 남긴다.** `document`는 upsert로 덮어써서 모델이 실제로 본 스냅샷이 이 행
    말고는 남지 않는다.
    """

    __tablename__ = "kospi_tool_call"
    __table_args__ = (
        UniqueConstraint("llm_run_id", "seq", name="uq_kospi_tool_call_natural_key"),
        CheckConstraint(
            "error_kind IS NULL OR error_kind IN"
            " ('unknown_tool', 'validation', 'limit', 'execution', 'cancelled')",
            name="ck_kospi_tool_call_error_kind",
        ),
        # 결과나 오류 중 하나는 있어야 한다. 둘 다 없는 행은 무슨 일이 있었는지 말하지 않는다.
        CheckConstraint(
            "result IS NOT NULL OR error IS NOT NULL",
            name="ck_kospi_tool_call_outcome",
        ),
        table_options(
            comment="코스피 전망·관찰 대화 안의 툴 호출 하나. 인자와 결과 전문을 남긴다",
            database="default",
        ),
    )

    llm_run_id: Mapped[int] = mapped_column(
        BigInteger,
        ForeignKey("kospi_llm_run.id", ondelete="CASCADE"),
        nullable=False,
        comment="이 호출이 속한 대화",
    )
    seq: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="그 대화 안의 순번. 1부터 빈 곳 없이 채워진다"
    )
    round_no: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="몇 번째 조사 왕복이었나. 한 왕복에 여러 호출이 묶인다"
    )
    tool_call_id: Mapped[str] = mapped_column(
        Text, nullable=False, comment="제공처가 준 호출 id. 요청과 결과를 잇는 값이다"
    )
    tool_name: Mapped[str] = mapped_column(
        Text, nullable=False, comment="모델이 부른 툴 이름. 모르는 이름일 수 있다"
    )
    arguments: Mapped[dict] = mapped_column(
        JSONB, nullable=False, comment="모델이 보낸 인자 그대로. 검증 전이라 스키마와 어긋날 수 있다"
    )
    validated_arguments: Mapped[dict | None] = mapped_column(
        JSONB,
        nullable=True,
        comment="검증을 통과해 함수에 들어간 인자. NULL이면 함수에 닿지 못했다는 뜻이다",
    )
    requested_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, comment="모델이 이 호출을 요청한 시각(UTC)"
    )
    duration_ms: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="함수 실행에 걸린 시간(밀리초). 함수에 못 닿았으면 NULL"
    )
    result: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="툴이 돌려준 본문 전문. 모델이 실제로 본 스냅샷이다"
    )
    result_chars: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default="0", comment="그 본문의 문자 수. 예산 분석의 재료다"
    )
    error_kind: Mapped[KospiToolCallErrorKind | None] = mapped_column(
        _enum_column(KospiToolCallErrorKind),
        nullable=True,
        comment=(
            "실패 종류(unknown_tool·validation은 함수에 못 닿음, limit은 우리 상한, "
            "execution은 실행 중 예외, cancelled는 sibling 실패로 시작조차 못 함)"
        ),
    )
    error: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="모델에게 돌아간 오류 문자열"
    )
    delivered: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        server_default="false",
        comment=(
            "결과가 모델 대화에 실제로 돌아갔나. sibling 실패로 버려진 결과는 오류가 아니라 "
            "'모델만 못 봤다'이고, 인용 분석이 그 구분 위에 선다"
        ),
    )
