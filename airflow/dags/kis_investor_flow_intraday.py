...
잘못된 시장 코드는 오류가 아니라 값 0으로 온다(실측). 그래서 수집기 Enum에는 확인된
코스피(`KSP/0001`)와 코스닥(`KSQ/1001`)만 있고, **모든 값이 0인 응답은 실패로 다룬다.**
코드의 근거는 `modules/collectors/kis_investor_flow.py`의 "잘못된 시장 코드가 0으로 온다"
절에 있다.
