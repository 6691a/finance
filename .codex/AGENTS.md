...
- **금리 전용 테이블이 아니다.** `kind`가 국채(`government_bond`), 단기 자금시장 금리(`money_market`), 중앙은행 정책금리(`policy_rate`), 물가지수(`price_index`), 실물활동(`activity`) 다섯을 가른다. 정책금리는 `money_market`이 아니다 — 시장이 만드는 값과 중앙은행이 정하는 값을 한 축에 두면 시장금리 패널이 정책금리 계단을 함께 그린다. 조회하는 쪽은 `kind`를 반드시 건다. 단위가 다른 값이 한 축에 섞이면 화면이 조용히 거짓말을 한다.
