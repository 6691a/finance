...
```
<파일 뿌리>/documents/<sha256[:2]>/<sha256><확장자>
```

DB에는 뿌리(`/opt/airflow/files`)를 뺀 **상대경로**만 남긴다. 마운트 지점이 바뀌어도 행을
고치지 않는다.

**경로를 문서 ID가 아니라 내용 해시로 만든다.** 초안은
`<출처>/<연>/<월>/<문서ID>-<순서>.<확장자>`였는데, 그러면 서로 다른 문서가 가리키는 같은
파일이 두 벌 저장되고 같은 문서를 다시 집을 때 순서가 밀리면 파일이 늘어난다. 해시로 두면
같은 바이트는 언제나 한 자리다. 앞 두 글자로 디렉터리를 갈라 한 폴더에 수만 개가 쌓이지
않게 한다.
