...
_12:35 KST 6,825.11 기준 · 전일 종가 대비 현재까지 -1.26%_
_전일 종가 대비 마감 예상 -1.96%_
> (하락 이유 …)
```
...
**예상 마감가(원·포인트)는 찍지 않는다** — 프롬프트가 모델에게 금지한 목표가를 우리가
계산해서 내보내는 꼴이 된다.

#### 2026-08-31 개정 — 분모를 글자로 적고 마감 예상 등락률을 한 줄 더한다

첫 판의 `오늘 여기까지 -2.29%`는 **분모를 말하지 않았다.** 국내 정규장은 개장 갭이 있어
전일 종가 대비와 오늘 시가 대비가 다른 값인데, 읽는 쪽이 시가 대비로 읽었다(그날 실제
오독). 채점 분모가 전일 종가라 그 오독은 예측 크기의 해석까지 바꾼다. 그래서
`전일 종가 대비 현재까지`로 분모를 문장에 넣는다.

같은 오독의 나머지 절반이 축의 덧셈이었다 — `현재까지 -2.29%` 아래의 `▼ 하락 1.0%`가 다시
하루 등락으로 읽혔다. 그 덧셈을 읽는 쪽에 시키지 않고 `_전일 종가 대비 마감 예상 -3.34%_`를
한 줄 더 찍는다(`render.py`의 `_close_line`).

- **가격이 아니라 등락률이다.** 위 문단이 금지한 것은 예상 마감가이고, 이 줄은 모델이 이미
  낸 크기를 전일 종가 기준으로 옮겨 적은 것이다. 새 예측을 만들지 않는다.
- 장중만 찍는다. 장전은 `base_return_pct`가 0이라 결론 줄의 크기와 같은 값이 된다.
- `flat`과 크기 없는 행은 더할 것이 없어 빠지고, 남는 방향이 없으면 줄 자체가 없다.
- 폭(`±%p`)은 결론 줄이 이미 말했다. 다시 적으면 같은 값이 한 화면에 두 번이다.
- 덧셈은 곱셈(`(1+a)(1+b)-1`)이 아니라 **단순 합**이다. -2.29%와 -1.0%에서 둘의 차이는
  0.02%p이고 예측 폭이 ±0.6%p라 그 안에서 뜻이 없다.

같은 문장을 조회 API 설명(`apps/api/schemas/thesis.py`), 컬럼 주석(리비전 `d9a41c7e05b3`),
사후 해설 프롬프트(`thesis/outcomes.py`의 `직전 종가` → `전일 종가`)에서 함께 고쳤다.
