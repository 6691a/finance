...
- 날짜: 2026-08-26
- 상태: 구현 완료 (2026-08-26). **넷으로 시작한 슬롯이 2026-08-28에 하나(`intraday_midday`)로
  줄었다** — 채점 84건에서 나머지 셋이 균등 추측보다 나빴다. 1절 표와 [TUNING.md](TUNING.md)를 본다.
  아래 본문의 "넷"은 설계 당시의 기록이고 지우지 않는다
...
## 1. 슬롯 — 넷으로 시작해 하나가 남았다

| 슬롯 | KST | 상태 |
| --- | --- | --- |
| `intraday_midday` | 12:35 | **남아 있는 유일한 장중 슬롯.** `document_assessment_hourly`(매시 :25) 뒤라 12:05 수집분이 근거 후보에 든다 |
| ~~`intraday_morning`~~ | ~~10:35~~ | **2026-08-28 제거.** 채점 Brier 0.721로 균등 추측(0.667)보다 나빴다 |
| ~~`intraday_afternoon`~~ | ~~14:35~~ | **2026-08-27 제거.** `pre_close`(15:00)와 25분 차이라 같은 창을 두 번 물었다. Brier 0.719 |
| ~~`pre_close`~~ | ~~15:00~~ | **2026-08-28 제거.** 창이 30분뿐인데 Brier 0.798로 넷 중 가장 나빴고 방향 적중이 17%였다 |

**제거는 셋 다 enum 값을 남긴다.** 저장된 행을 `market_thesis_review`가 채점·해설하고 API가
조회한다. 라벨(`thesis.domain.SLOT_LABELS`)도 돌던 때의 시각으로 남는다.

근거는 2026-08-21~28 채점 84건이다. 남은 `intraday_midday`(0.595)와 장전 `pre_open`(0.634)만
균등 추측보다 나았고, 전체 평균은 0.668로 균등(0.667)과 구분되지 않았다. 판단 절차와 재측정
방법은 [TUNING.md](TUNING.md)에 있다. **성적이 오르면 다시 늘릴 수 있는 손잡이다** —
`INTRADAY_SLOT_TIMES` 표에 한 줄을 더하고 DAG cron을 맞추면 된다.
