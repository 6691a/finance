...
class KisTimeWindowError(KisResultError):
    """아직 그 구간을 받아 주지 않는 시각이다. **재시도로 풀릴 문제가 아니다.**

    확정 일별 수급(`FHPTJ04160001`)은 **당일을 구간 끝으로 주면** KST 15:40 전에 `rt_cd=2`와
    `TIME LIMIT 00:00 ~ 15:40`으로 거절한다. 마감 확정이 그 시각에 나오기 때문이다.

    **과거 구간은 시각과 무관하게 받아 준다**(2026-08-25 실측: 13:36에 `end_date=2025-01-14`
    조회가 성공했고, 같은 날 14:11에 `end_date=2026-08-25` 조회는 거절당했다). 그래서 백필은
    아무 때나 돌 수 있고, 당일치는 15:40 이후에만 들어온다.

    재시도는 그 시각이 될 때까지 같은 답을 받을 뿐이라 태스크의 재시도 예산만 태운다.
    """
