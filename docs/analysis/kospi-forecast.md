...
> - 한계: 2026-09-03 오후 급락(일본 금리 루머)은 일본·중국 지수가 요인에 없어 말할 수 없었고, 추가는 판 동결 뒤다. 루머를 미리 맞히는 것은 목표가 아니다. AI가 요인을 골라 보던 첫 판은 15개 중 8개만 기록이 남아 안 본 요인의 가중치가 얼어붙었다 — 고친 판은 코드가 15개를 다 주며 판 6 동결이 끝나는 날 배포한다. 20영업일 뒤 관계가 수렴하지 않으면 핵심 가정이 틀린 것이고 접는다.
...
- 상태: **운영 중.** 2026-09-03에 슬롯 셋과 관찰이 돌았고 채점까지 확인했다. 관계 그래프는
  ...
  §8.9). 코드는 전망 판 8·관찰 판 3(2026-09-06, §8.10과 출력 형식의 스키마 이동)이고 **판 6 동결이 끝나는 날 배포한다.**
...
| 조용한 날 관찰 커버리지 | **2026-09-06 구현(§8.10)** | 코드가 요인 15개 값을 표로 주고 모델이 줄마다 답한다. 관찰 판 2·전망 판 7. **배포는 판 6 동결이 끝나는 날** |
...
## 8.10 관찰이 요인을 고른다 — 선택 편향과 얼어붙는 가중치 (구현 2026-09-06, 배포 대기)

- 날짜: 2026-09-06(설계·구현, 워크트리 `feature-kospi-observation-coverage`). **배포는 판 6
  동결이 끝나는 2026-10 초에** 한다 — 머지·`just migrate upgrade head`(리비전 `d4f8a2b7c913`)·
  파일 반영을 같은 날 하고 그날부터 20영업일을 다시 센다.
...
바뀐다.** §8.9와 같은 이유로 전망 판도 함께 올린다(6 → 7). 판 6 동결이 끝나는 2026-10 초에
둘을 한 날 올리고 그날부터 20영업일을 다시 센다. 그 전에 고치면 표본이 갈린다.
