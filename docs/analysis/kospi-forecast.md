...
- 날짜: 2026-09-02(작성), 2026-09-03(운영 기동·옛 기능 삭제), 2026-09-06(§8.10 구현), 2026-09-09(§8.11 구현)
- 상태: **운영 중.** 2026-09-03에 슬롯 셋과 관찰이 돌았고 채점까지 확인했다. 관계 그래프는
  8거래일(8/24~9/2)을 백필해 관측 34건이 쌓였다. 운영은 전망 판 6·관찰 판 1(2026-09-04,
  §8.9). 2026-09-06에 전망 판 7·관찰 판 2(§8.10)와 판 8·관찰 판 3(출력 형식을 응답 모델로
  옮김)을 배포했다. 20영업일 시계는 2026-09-07 장전부터 다시 센다.
  **§8.11(안 바뀐 값의 `none`)은 2026-09-09에 구현했고 배포 전이다**(리비전 `e2a7c5d19b04`).
...
- 산출물: `apps/models/analysis/kospi.py`(표 셋)와 수기 리비전 `a1c74f0b8e35`,
  ...
  `test_kospi_models`·`test_kospi_schema`, 125개). 저장소 전체는 2,376개(2026-09-09).
...
earlier_slots_today:
  pre_open 08:35  up  +1.2% ± 0.6%  이유 3건 (방향 · factor · 문장)
```
...
- `earlier_slots_today`는 `kospi_forecast`의 같은 날 앞 슬롯 행이다. 프롬프트가 "이것은
  오늘 앞선 시각의 판단이고 정답이 아니다"를 밝힌다. 이유마다 **요인 코드**(`factor`)와
  `memory_id`가 문장 옆에 붙는다(판 5, §8.8) — 장중 슬롯이 그 요인을 `factor_history`로
  다시 불러 "아직 작용하나"를 판단하는 손잡이다. 검증 규칙은 그대로다: `slot_ref`만으로는
  이유가 남지 않고, 이어받은 요인도 이 실행에서 조회했어야 한다.
...
| `recent_news` | `hours ≤ 48` | 평가된 문서 상위 N — 제목·발행 시각·`direction`·`value_score`·태그·평가 요약. `value_score`가 `NEWS_MIN_VALUE_SCORE`(4) 미만이면 오지 않는다. `total`(하한을 넘은 전체)과 `shown`(돌려준 수)을 함께 실어 잘렸는지 보인다 |
...
포함) + 관계 표 + 활성 메모 전부 + **숫자 요인 15개의 그날 값 표(`factor_moves`, 판 2·§8.10)**.
툴은 뉴스·공시 둘이고(`factor_history`는 관찰에서 빠졌다 — 값이 표에 있다) `as_of_at`은
19:00 KST.
...
{"observations": [
   {"factor": "FOREIGN_NET_BUY", "sign": "same", "strength": 3,
    "note": "외국인 8백만주 순매수가 반도체 중심 상승을 주도"},
   {"factor": "US10Y", "sign": "inverse", "strength": 1,
    "note": "10년물 +6bp였지만 수급이 눌렀다"},
   {"factor": "KRBASE", "sign": "none", "strength": 0, "note": "변화 없음"},
   "... 표의 15줄 전부"],
 "memories": [
...
- **값을 본 요인의 관찰만 남긴다.** 표(`factor_moves`)에 실린 요인은 봤다. 뉴스·공시는 표에
  없어 툴 호출이 증거다. 둘 다 아니면 버리고 센다.
- **표의 줄이 빠지면 0으로 채우지 않는다.** 빠진 요인 이름을 실어 한 번 되묻고, 그래도 빠지면
  기록하지 않고 `kospi_llm_run.observations_unanswered`로 센다(§8.10).
- `none`은 세기 0이다. 세기를 적어 와도 0으로 둔다.
- 오늘 `abs(actual_change_pct) ≥ 0.5%`인데 **이어진 관찰(`none` 제외)**이 0건이면 교정을 한 번
  묻고, 그래도 0이면 **태스크를 실패시킨다.** 움직인 날에 이유가 없는 것은 답이 아니다.
  0.5% 미만이면 전부 `none`을 허용하고 원장에 남긴다.
...
```mermaid
flowchart TD
    START([START]) --> load[load_state<br/>오늘 등락률 · 15일 봉 · 관계 표 · 활성 메모<br/>숫자 요인 15개의 그날 값 표]
    load --> investigate[investigate<br/>모델 + 툴 2개<br/>recent_news · recent_disclosures]
    investigate -->|tool_calls| tools[tools<br/>ToolNode]
    tools --> investigate
    investigate -->|끝| answer[answer<br/>response_format<br/>ReviewAnswer]
    answer --> verify[verify<br/>표에 있거나 조회한 요인만 남긴다 · none은 세기 0<br/>빠진 줄 셈 · 메모 상한·중복 · 리뷰 누락 셈]
    verify -->|"빠진 줄 있음, 또는<br/>이어진 관찰 0건인데 |등락| ≥ 0.5%<br/>& attempts == 0"| repair[repair]
    repair --> answer
    verify -->|"이어진 관찰 0건인데 |등락| ≥ 0.5%<br/>& attempts == 1"| fail[[EmptyReviewError<br/>태스크 실패]]
...
| 조용한 날 관찰 커버리지 | **2026-09-06 구현·배포(§8.10)** | 코드가 요인 15개 값을 표로 주고 모델이 줄마다 답한다. 관찰 판 2·전망 판 7 |
...
## 8.7 아시아 축이 요인에 없다 — 2026-09-03 14:10 급락이 드러낸 것 (미구현)

### 그날 실제로 일어난 일

```
KST 14:00  6,645     정상
KST 14:10  6,592     하락 시작
KST 14:20  6,478     -2.5%  (20분 만에)
KST 14:30  6,497
KST 14:50  6,581     되돌림
KST 15:30  6,579     +0.26% 마감  ← 아무 일 없었던 것처럼
```

한국·중국·일본 지수가 **같은 시각에 함께** 움직였다. 전해 들은 원인은 **일본은행 50bp
빅스텝 인상 루머**이고, 프로그램 매물이 쏟아졌다가 루머라 바로 되돌렸다.

### 왜 우리 전망이 이것을 말할 수 없었나

**① 일본·중국이 `Factor`에 없다. 데이터는 이미 있는데.**

`quote_daily`와 `index_daily`에 **`NIKKEI225`·`SSE_COMP`·`HSI`·`TAIEX`·`USDJPY`·`USDCNH`가
매일 들어오고 있다**(2026-09-04 실측). 그런데 §3.1 요인 목록 열일곱에 아시아가 하나도 없다 —
미국(SP500·NASDAQ·SOX·VIX·US10Y·DXY)과 국내(수급 셋·KTB10Y·KRBASE·USDKRW·삼성·하이닉스)뿐이다.

그래서 모델은 "엔이 급등 중이다"·"닛케이가 같이 빠졌다"를 **값으로 조회할 방법이 없다.**
`USDJPY`는 문서 태그로는 최근 7일 95건이 붙었는데 `factor_history`로는 못 부른다. 관계
그래프에도 안 쌓이므로 "엔 급등이 코스피와 어떻게 움직였나"의 가중치가 영영 생기지 않는다.

**② 재료는 오전에 다 들어와 있었다.** 그날 수집된 문서 중:

```
01:05 수집  달러-엔 돌연 급락…장중 158.202엔까지 굴러떨어져(상보)          6점
09:05 수집  [외환] 日 '레이트 체크' 의심에 1,350원대로 급락…연저점 또 경신  8점
12:05 수집  채권 딜러들, 글로벌 현장에서 보는 '엔 캐리' 청산 징후는         8점
```

**엔 캐리 청산 징후가 8점으로 이미 프롬프트에 실릴 수 있었다.** 그런데 툴이 제목과 평가
요약만 주고 본문을 안 줘서(6.1), 모델이 "그래서 오늘 오후에 무슨 일이 나나"를 읽을 재료가
없다. 8점짜리 제목 한 줄로는 지수 요인과 이어 붙일 수 없다.

**③ 슬롯과 수집 주기가 어긋나 `pre_close`가 급락을 못 봤다.**

```
14:05  마지막 수집 배치        ← pre_close(14:35)가 보는 마지막 문서
14:10  급락 시작
14:20  저점 6,478
14:35  pre_close 슬롯 기준 시각  ← 급락은 봉으로 보이지만 이유는 문서에 없다
14:48  급락을 다룬 첫 기사 발행
15:05  그 기사 수집             ← 슬롯이 30분 뒤에 돌았어도 못 봤다
```

문서 수집이 매시 :05이고 슬롯이 :35라, **슬롯은 항상 30분 전까지의 뉴스만 본다.** 장중에
터진 사건은 구조적으로 다음 슬롯에서야 보이고, `pre_close`는 다음 슬롯이 없다.

**④ 사건 기사가 시황 기사보다 낮은 점수를 받았다.**

```
7점  코스피 하락 전환, 6,500선 턱걸이…외인·개인·기관 1조4천억 순매도   (14:48 발행)
6점  코스피, 돌연 급락 후 6,500대 되돌림…0.26% 오른 6,579.48 마감      (15:34 발행)
7점  국내 주식 마감 시황 - 30분 만에 3% 급락 후 회복(9월 3일) - 신한투자증권
```

그날 코스피 최대 사건을 다룬 셋이 6~7점인데, 같은 창의 `8월 수출` 리포트 넷은 전부 8점이다.
[economic-document-archive-design.md](economic-document-archive-design.md) §6.8이 고치려는
것이 정확히 이 순서다.

### 무엇을 한다

**넷 다 서로 다른 자리이고 크기가 다르다.** 값싼 것부터.

| # | 무엇 | 크기 | 언제 |
| --- | --- | --- | --- |
| 1 | 아시아 요인 넷을 `Factor`에 더한다 — `NIKKEI225`·`SSE_COMP`·`USDJPY`·`USDCNH` | §3.1 표 네 줄, `Factor` 네 값, `FACTOR_SPECS` 네 항목. **SQL은 안 는다**(전부 `QUOTE_DAILY` 자리다) | 동결 뒤 |
| 2 | 툴이 본문 앞부분을 준다 — 상위 10건에 `left(body, 300)` | `select_news.sql` 한 칸, `NewsRow` 한 필드 | §6.8과 함께 |
| 3 | 장중 슬롯 앞에 문서 수집을 한 번 더 돌린다 | `document_ingestion_hourly`를 :05·:30 둘로, 또는 장중만 별도 cron | 별건 |
| 4 | 사건 기사가 시황 리포트보다 위로 오게 | [§6.8](economic-document-archive-design.md) 그 자체 | 진행 중 |

**1번은 동결 뒤다.** 요인이 열일곱에서 스물하나가 되면 `factor_history`로 볼 수 있는 것이
열다섯에서 열아홉이 되고, 툴 상한 25에서 문서 툴 둘을 빼면 스물셋이라 **아직 들어간다.**
다만 모델이 보는 것이 크게 달라져 20영업일 표본을 둘로 가른다.

**요인을 더할 때 확인할 것 둘.** ① `NIKKEI225`·`SSE_COMP`는 `index_daily`에도 `quote_daily`에도
있다 — 어느 쪽을 원본으로 쓸지 정하고 §3.1에 적는다. ② 최근 5일 실측에서 둘 다 마지막
행이 `2026-09-02`로 하루 늦다. 장전 슬롯이 "어제 닛케이"를 못 보는 날이 생기는지, 아니면
수집이 밀린 것인지 먼저 재야 한다. **재기 전에 요인으로 올리면 빈 표가 프롬프트에 실린다.**

### 이 절이 답하지 못하는 것

**루머는 기사가 되기 전에 가격에 반영된다.** 14:10에 던진 프로그램은 어느 매체보다 빨랐고,
우리가 무슨 짓을 해도 그 순서는 못 뒤집는다. 1~3번이 하는 일은 "급락을 미리 맞힌다"가 아니라
**"엔 급등과 닛케이 동조가 진행 중이라는 것을 값으로 보고 있다"**까지다. 그다음 슬롯과
장후 관찰이 그 관계를 `OBSERVED` 엣지로 쌓으면, 다음번 같은 조합에서 폭을 넓게 부르는 것이
이 설계가 낼 수 있는 최선이다. 그것을 못 하고 있는 지금이 문제이지, 루머를 맞히지 못하는
것은 문제가 아니다.

## 8.8 장중이 장전 재료를 말없이 버린다 — 2026-09-04 실측과 판 5

### 본 것

09-04 장전(08:35)이 이유 14건을 냈다. 상방이 SP500·NASDAQ·US10Y·VIX·DXY·USDKRW와 메모 둘,
하방이 SOX·WTI·삼성·하이닉스와 메모 하나. 장중(11:35)은 8건이었고 장전 상방 재료의 운명은
이랬다.

| 장전 상방 재료 | 장중에서 |
| --- | --- |
| SP500·NASDAQ | 하나로 접어 "이미 +1.20% 소화했다, 다시 더하지 않는다"(`slot_ref: pre_open`) |
| US10Y·VIX·DXY | 사라짐. 재조회도 언급도 없음 |
| USDKRW | **하방으로 뒤집힘**(원화 강세가 수출주 부담) |
| 메모 둘 | 사라짐 |

전날(09-03) `pre_close`도 SP500을 "갭으로 소진"으로 정리했다. 코드가 장전 자료를 빼는 것이
아니라 **판 4의 장중 지시문이 한쪽으로만 밀었다** — "이미 일어난 것을 다시 세지 마라",
"개장 뒤 재료가 아침에 없던 것이다", "앞 슬롯은 정답이 아니다". 반대편 문장("밤사이 재료가
하루 내내 또는 며칠 이어질 수 있다")이 없었고, 재료의 지속 시간을 말하는 자리가 프롬프트
어디에도 없었다. 관계 가중치의 감쇠(§3.3)는 과거 관계에만 있고 오늘 재료의 지속성에는
아무 장치가 없다.

### 왜 문제인가

예측 축이 "현재가 → 종가"라 갭에 실린 만큼을 빼는 것은 설계 의도다(§2.1). 그러나 **"가격에
얼마나 반영됐나"와 "재료가 아직 작용하나"는 다른 질문**인데 판 4가 둘을 한 문장으로 뭉갰다.
금리·유가·환율은 시가 갭 하나로 끝나지 않는다.

### 고친 것 (판 5, 2026-09-04)

- **지시문.** 장중 문단에 "가격이 움직였다고 재료가 끝난 것은 아니다"를 넣고 두 질문을 갈랐다.
  앞 슬롯 이유의 요인마다 이어받을지 소진됐는지 **하나씩** 판단하고, 이어받으면
  `factor_history`로 재조회한 뒤 `slot_ref`를 붙이고, 소진·반전이면 **그것도 이유로 적는다.**
  말없이 빼면 빠진 것과 판단한 것이 기록에서 구별되지 않는다.
- **관측 상태.** `EarlierSlot.reasons`가 문장 튜플에서 `EarlierReason`(방향·문장·`factor`·
  `memory_id`)으로 바뀌었다. 문장만 주면 모델이 "S&P500이 올랐다"에서 `SP500`을 다시 짐작해야
  했다. 모델이 보는 것이 바뀌므로 프롬프트와 같은 판에 묶었다 — 손잡이 둘을 한 판에 당긴
  것이고, 그 사실을 여기 적는다.
- **검증은 안 건드렸다.** 이어받은 요인도 이 실행에서 조회했어야 남는다. `MAX_TOOL_CALLS` 25에
  요인 15개라 재조회 예산은 있다.

### 무엇을 보나

20영업일 시계가 판 5에서 다시 시작한다([kospi-evaluation.md](kospi-evaluation.md) §0).
그 사이 볼 것은 둘이다 — 장중 이유 중 `slot_ref`가 붙은 비율(0에 가까우면 지시문이 안
먹었다), 그리고 장전 요인 중 장중 `kospi_tool_call`에 다시 나타나는 비율.

## 8.9 좁은 창에서는 점수 컷이 없다 — 2026-09-04 실측과 판 6

### 본 것

`recent_news`는 `value_score DESC`로 정렬해 상위 30건만 준다. 2026-09-03에 점수 하한 인자를
없애면서 근거로 "24시간 창의 후보가 303건이라 `LIMIT`이 알아서 상위만 남긴다"를 적었다.
그 근거는 창이 넓을 때만 참이었다. 창을 좁히면 후보가 30건 밑으로 떨어지고 `LIMIT`이
아무것도 자르지 않아 **창 안 전부가 그대로 모델에 간다.**

운영 DB 5일(08-31~09-04) 실측이다. 30컷 최저 점수가 곧 모델이 본 가장 낮은 점수다.

| 슬롯 | 창 | 후보 | 30컷 최저 점수 |
| --- | --- | --- | --- |
| 08:35 | 24h | 258~311건 | 7~8 |
| 08:35 | 6h | 14~27건 | **0~2** |
| 08:35 | 3h | 14~21건 | **1~2** |
| 13:30 | 1h | 5~10건 | **2~4** |
...
```
0  모두가 아는 매크로 악재 말고 진짜 우리가 모르는 하락 이유가 있나
1  [부고] 전춘봉(세연회계법인 이사)씨 부친상
2  건축공간연구원, 김제서 '2026 제3차 아우리 건축도시포럼' 개최
3  라이나손보 신임 대표에 김일평 전 삼성화재 부사장 선임
```

**점수는 제 일을 하고 있었다.** 부고·인사·행사가 0~3점에 정확히 모였다. 문제는 그 점수를
쓰는 자리가 `ORDER BY`뿐이라 후보가 30건 밑이면 정렬이 순서만 바꾸고 아무도 안 버린다는
것이다. 장전 슬롯이 제일 취약하다 — 밤사이라 후보가 원래 10~27건이다.

### 고친 것

- `NEWS_MIN_VALUE_SCORE = 4`를 `kospi/domain.py`에 두고 `select_news.sql`이 술어로 건다.
  **모델이 고르는 인자가 아니다.** 2026-09-03에 없앤 것은 모델이 돌린다고 믿지만 답을 안
  바꾸던 손잡이였고, 이것은 코드가 정하는 하한이다.
- `total`도 하한 뒤로 센다. 잘림 표시가 "네가 볼 수 있었던 것 중 몇 건"이어야 한다.
- 툴 설명에 하한을 적었다. 안 적으면 모델이 "그런 기사는 없었다"를 하한 때문에 못 본 것에도
  쓴다.
- 4는 0~8 눈금의 절반이다. 같은 5일 실측에서 하한 4가 창을 비우지 않았다 — 1시간 창도 3건
  이상 남았고, 24시간 창은 `LIMIT`이 이미 7점 위를 남기고 있어 술어가 아무 일도 하지 않는다.

### 판을 올린 이유

**프롬프트 문장은 한 글자도 안 바뀌었다.** 바뀐 것은 모델이 보는 증거다. 같은 문장을 다른
기사 묶음으로 돌린 실행을 한 판으로 세면 채점이 섞이므로 판 6으로 올린다.
`test_prompt_versions.py`의 해시는 판 5와 같다 — 그 표가 잠그는 것은 문장이고, 판을 올릴
이유는 문장 말고도 있다.

**20영업일 시계가 여기서 또 처음부터 간다**([kospi-evaluation.md](kospi-evaluation.md) §0).
판 5는 실행 하나(09-04 장전)만 갖고 끝난다.

### 무엇을 보나

`kospi_tool_call`의 `recent_news` 응답에서 `total`과 `shown`, 그리고 인용된 기사의 최저
`value_score`. 하한이 창을 비우는 슬롯이 있으면 그때는 하한이 아니라 창 길이를 본다.

## 8.10 관찰이 요인을 고른다 — 선택 편향과 얼어붙는 가중치 (구현·배포 2026-09-06)

- 날짜: 2026-09-06(설계·구현·배포, 워크트리 `feature-kospi-observation-coverage`). 머지·
  `just migrate upgrade head`(리비전 `d4f8a2b7c913`)·pull을 같은 날 했다. **판 6 동결의 끝을
  기다리지 않았다**(사용자 결정) — 커버리지 구멍이 매 거래일 관측을 잃고 있어 기다리는 비용이
  더 컸다. 20영업일 시계는 첫 관찰 판 2가 도는 2026-09-07부터 다시 센다.
- 닫는 것: §9.2의 6("조용한 날의 관찰 커버리지"). [kospi-evaluation.md](kospi-evaluation.md)
  §4의 "관측 커버리지 — 2026-09-03 기준 15개 중 8개만 관측"이 이 절의 실측이다.

### 왜 이 절이 있나

사용자가 던진 질문은 "피드백 루프는 **선택된 것만 강화하고, 선택받지 못한 것은 영구히
배제되지 않나**"였다. 지금은 수급이 시장을 움직이지만 석 달 전에는 반도체 기대가 움직였고,
루프가 수급만 계속 보면 반도체가 돌아와도 알아채지 못한다는 걱정이다.

이 루프는 강화학습이 아니라서 그 문제가 **절반만** 있다. 관계 가중치의 원료는 "전망이 무엇을
인용했나"가 아니라 "장후에 시장이 실제로 어떻게 움직였나"다(§3). 전망이 수급만 인용해도
장후 관찰이 SOX를 봤으면 SOX 엣지는 쌓인다. 전망 쪽의 편향은 판단이고, 학습은 장후에서만
일어난다.

**그런데 새는 자리가 둘 있고, 둘이 맞물려 사용자가 말한 순환을 만든다.**

### 새는 자리 1 — 장후 관찰이 요인을 스스로 고른다

`kospi_review.yaml`은 "툴로 값을 조회한 요인만 적을 수 있다"고 한다. 조회는 `factor_history`
툴로 **요인 하나씩** 모델이 부른다(`toolbox.py`). 모델은 관계 표에서 무거운 요인부터 부르고,
가벼운 요인이나 `n_obs=0`인 요인은 안 부른다. 안 부르면 그 요인에 대해 "오늘 상관없었다"는
기록이 남는 것이 아니라 **아무 기록도 남지 않는다.**

```
19:00 장후 관찰
  관계 표:  외국인 순매수 +0.80 (12관측)   SOX +0.90 (10관측, 마지막 06-12)   VIX -0.33 ...
  모델이 factor_history를 부른다:  외국인 ✓  기관 ✓  원달러 ✓  ...  SOX — 안 부름
  답:  외국인 same·3, 기관 same·2
  저장:  SOX는 오늘 행이 없다
```

2026-09-03 실측이 그 결과다 — 요인 15개 중 8개만 관측이 있었고 US10Y·DXY·KTB10Y·KRBASE·
SP500·NASDAQ·VIX는 0이었다. 미국 지수 셋이 0인 날은 "안 움직인 날"이 아니라 "안 본 날"이다.

### 새는 자리 2 — 관찰이 끊기면 가중치가 멈춘다

`domain.relation_weight`는 **가중 평균**이다. 분자와 분모가 같은 감쇠를 타므로 나눈 뒤에는
나이가 사라진다. 6월에 SOX가 `same·3`으로 열 번 관측되고 그 뒤 한 번도 관측이 없으면 오늘
SOX의 가중치는 여전히 **+1.0**이다(`last_date`만 6월). 90일 조회 창(`RELATION_LOOKBACK_DAYS`)
밖으로 떨어질 때까지 그대로다.

반감기 5일은 **관찰이 계속 들어올 때** 뒤집힘을 따라간다. 관찰이 끊기면 옛 값이 얼어붙는다.
그래서 둘이 맞물린다.

```
무거운 요인 → 모델이 매일 조회 → 관측이 쌓임 → 계속 무거움
가벼운 요인 → 조회 안 됨      → 관측 없음   → 옛 값 그대로(0이거나 옛날의 +1.0) → 다음 날도 조회 안 됨
```

"수급이 이제 안 먹힌다"도 모델이 수급을 **계속 조회해야만** 숫자에 반영된다. 반도체가
돌아오는 것도, 수급이 죽는 것도 같은 문에 걸려 있다.

### 무엇을 한다 — 모델이 고르지 않게 한다

**관찰은 모델이 요인을 고르는 단계를 없앤다.** 코드가 숫자 요인 15개의 그날 변화를 SQL로
한 번에 뽑아 `ReviewState`에 표로 싣고, 모델은 **줄마다 하나씩** 판정한다. 뉴스·공시 둘은
내용을 읽어야 하는 요인이라 지금처럼 툴로 남고, 조회한 경우에만 관찰을 받는 규칙도 그대로다.

```
바꾸기 전:  모델이 factor_history로 요인을 골라 조회 → 조회한 것만 판정 → 나머지는 기록 없음
바꾼 뒤:    코드가 15개 값을 표로 준다 → 모델은 15줄 다 판정 → 15줄 다 기록
```

답 한 줄은 셋 중 하나다. `none`이 새로 생긴다.

| 답 | 뜻 | 가중치 셈에 들어가는 값 |
| --- | --- | --- |
| `same · 세기` | 요인과 코스피가 같은 방향으로 갔고 오늘 움직임에 이만큼 작용했다 | +세기 |
| `inverse · 세기` | 반대 방향으로 갔다 | −세기 |
| `none` | **값은 봤는데** 오늘 코스피 움직임과 상관없었다 | 0 |

`none`을 0으로 평균에 넣으면 새는 자리 2가 함께 닫힌다. 안 먹히는 요인은 `none`이 쌓여
분모만 늘고 분자는 안 늘어서 반감기대로 0으로 내려간다. 다시 먹히기 시작하면 `same`이 쌓여
올라온다. "관측 없음(`n_obs=0`, 아직 모른다)"과 "관측했더니 무관(`none`, 안다)"이 처음으로
갈린다 — 지금은 둘이 같은 빈칸이다.

`ObservationSign`에 `NONE`이 늘고 `strength`는 `none`일 때 0이다. Neo4j 엣지는 지금처럼
하루에 요인당 하나이고(§3.2) `none`도 엣지다 — "봤다"는 기록이 있어야 나중에 "왜 0인가"를
되짚을 수 있다. `recent_signs`에도 그대로 실린다.

**모델이 표에 15줄이 있는데 14줄만 답하면** 0을 채우지 않는다. 확인하지 않은 것을 기록하지
않는다는 규칙이다. 메모 판정이 하는 것과 같은 모양으로 간다(§4.4).

- 빠진 줄이 있으면 **한 번 되묻는다**(빠진 요인 이름을 실어서). §6.2의 "큰 날 관찰 0건이면
  한 번 되묻기"와 같은 `repair` 자리다.
- 그래도 빠지면 그 줄은 기록하지 않고 원장 `kospi_llm_run`에 `observations_unanswered`로 센다.
  빠진 것을 세어 두면 "모델이 요인을 자꾸 빼먹는다"가 데이터로 보인다. 채워 넣으면 안 보인다.

### 관찰 그래프가 이렇게 바뀐다

`load_state`가 요인 값 표를 함께 만들고, `investigate`의 툴은 `recent_news`·
`recent_disclosures` 둘만 남는다. `verify`는 "조회한 요인만 남긴다"에서 "표에 있는 요인은 전부
답했나, 뉴스·공시는 조회했나"로 바뀐다. 나머지 노드는 그대로다(§7.2).

```
load_state        + 숫자 요인 15개의 그날 값 (SQL, factor_history와 같은 조회를 15번)
investigate       툴 2개 (뉴스 · 공시)
answer            ReviewAnswer — observations에 15줄 + 뉴스·공시
verify            빠진 줄 셈 · 뉴스·공시는 조회 여부 · 메모 상한·중복 · 리뷰 누락
repair (1회)      큰 날 0건 **또는 빠진 줄 있음**
write_graph       none 포함 MERGE
```

툴 왕복은 줄어든다 — 요인 조회 최대 15회가 0회가 되고 뉴스·공시 둘만 남는다. 프롬프트에
15줄 표가 늘어나는 것이 비용의 전부다.

### 모델이 받는 것과 내는 것

받는 표(`ReviewState.factor_moves`, 코드가 만든다):
...
내는 답(`observations`, 15줄 전부 + 뉴스·공시 중 조회한 것):

```json
{"observations": [
   {"factor": "FOREIGN_NET_BUY", "sign": "same",    "strength": 3, "note": "외국인 8백만주 순매수가 반도체 중심 상승을 주도"},
   {"factor": "SOX",             "sign": "same",    "strength": 2, "note": "전날 SOX +2.1%가 삼성·하이닉스 갭업으로 이어졌다"},
   {"factor": "US10Y",           "sign": "none",    "strength": 0, "note": "+6bp였지만 오늘 장은 금리를 보지 않았다"},
   {"factor": "KRBASE",          "sign": "none",    "strength": 0, "note": "변화 없음"},
   ...]}
```
...
- **안 본 요인에 낮은 점수(0)를 넣는 것.** "안 봤다"와 "봤는데 상관없었다"는 다른 사실이다.
  안 본 SOX에 0을 넣으면 확인하지 않은 것을 기록한 것이고, 그날 SOX가 진짜 장을 끌었을 수도
  있다. 순환도 못 끊는다 — 안 본 요인이 0으로 내려가면 다음 날 더 안 본다. 그래서 "안 봤으니
  낮게"가 아니라 "안 볼 수 없게"다.
- **웹 검색으로 보완하는 것.** 이 구멍은 정보가 모자라서가 아니라 이미 있는 정보를 다 안 봐서
  생긴다. 15개 요인의 그날 값은 전부 우리 표에 있다(`quote_daily`·`indicator_observation`·
  `stock_investor_trade_daily`·`market_investor_flow_snapshot`). 검색은 그 값을 밖에서 다시 사
  오는 것이고, 매일 다른 페이지가 와서 같은 판의 두 날을 비교할 수 없게 만들며, 원장에 "그때
  모델이 무엇을 봤나"를 남길 수 없다. 요인 목록 **밖**의 서술("반도체 기대")은 검색이 닿는
  자리가 맞지만 거기는 이미 문서 수집(`document`, `recent_news` 툴)이 있다 — 그쪽이 모자라면
  수집 출처를 늘린다. 국내 소스 우선 원칙과도 같다.
- **ε-탐색(전망에 무작위로 가벼운 요인을 인용시키는 것).** 전망 정확도를 깎아 얻는 정보가,
  장후에 15개를 공짜로 다 보는 것과 같다.
- **가중치 하한·상한 클램프.** 증상만 가리고 원인(커버리지)은 그대로다.

### 어휘 밖은 따로 센다

위로 **17개 안에서의** 부활은 보장된다. 못 부활하는 것은 `Factor` 목록에 없는 것이다.
"반도체 실적 기대"는 SOX·삼성·하이닉스로 반쯤 잡히지만 "기대" 자체는 `NEWS`에 뭉개진다.
§8.7의 "아시아 축이 요인에 없다"가 같은 종류였고 급락 뒤에야 알았다.

장후 답에 `unlisted_drivers: [자유 문장]` 칸 하나를 둔다 — "요인 목록에 없는데 오늘 움직인
것". 저장만 하고(`kospi_llm_run`에 JSONB) 가중치에는 안 넣는다. 20영업일 판정 때 이 칸을 세어
자주 나오는 것을 요인으로 승격한다. 어휘 확장은 사람이 하되, **어디를 늘려야 하는지를
데이터가 말하게 한다.** 지금은 사람이 급락 사후 분석에서 알아채야 한다.

### 판과 시점

셋 다 모델의 답을 바꾸는 변경이다 — `REVIEW_PROMPT_VERSION` 1 → 2. 전망 프롬프트 문장은
안 바뀌지만 **관계 표의 가중치와 `recent_signs`에 `none`이 실리므로 전망 모델이 보는 것이
바뀐다.** §8.9와 같은 이유로 전망 판도 함께 올린다(6 → 7). 둘을 2026-09-06에 한 날 배포했고
20영업일은 2026-09-07 장전부터 센다. 판 6은 표본을 채우지 못하고 끝난다 — 동결 규칙을 알고
깬 것이고 이유는 위 날짜 줄에 있다.

### 구현 (2026-09-06)

| 무엇 | 어디 |
| --- | --- |
| `ObservationSign.NONE`, `signed_strength`, 두 판 상수 | `kospi/domain.py` |
| `FactorMove`, `ReviewState.factor_moves` | `kospi/state.py` |
| `KospiToolbox.factor_moves()`(툴이 아니다 — 예산·원장에 안 남는다), `include_history=False` | `kospi/toolbox.py` |
| `ObservationAnswer.sign`에 `none`, `ReviewAnswer.unlisted_drivers`, 표 요인 허용·빠진 줄 셈·되묻기 | `kospi/generation.py` |
| 관찰이 툴 없는 툴박스로 표를 만들고 원장 칸 둘을 쓴다. 큰 날 판정은 `none` 제외 | `kospi/review.py` |
| `none`은 세기 0만 받는다 | `kospi/graph.py` |
| Slack은 `none` 줄을 숨기고 수만 꼬리에, 답 없는 요인은 이름을 보인다 | `kospi/render.py` |
| `kospi_llm_run.observations_unanswered`·`unlisted_drivers` | `apps/models/analysis/kospi.py`, 리비전 `d4f8a2b7c913`, `kospi_llm_run/finish.sql` |
| 프롬프트 판 2 | `prompts/kospi_review.yaml`, `test_prompt_versions.py` 해시 |

테스트 18개를 더했다(도메인 2, 파이프라인 12, 스키마 1, 그 밖 3). **실제 모델로 돌린 프로토타입은
없다** — 배포 전에 노트북으로 하루를 돌려 15줄이 다 오는지, `none` 비율이 어떤지 본다.

### 무엇을 보나

- **커버리지가 15/15인가.** `kospi_llm_run.observations_unanswered`가 0이어야 한다.
  [kospi-evaluation.md](kospi-evaluation.md) §4의 Cypher에서 관측 0인 요인이 사라져야 한다.
- **`none`의 비율.** 요인별 `none` 비율이 0이면 모델이 억지로 방향을 붙이는 것이고(§6.2의
  "억지로 요인을 붙이지 마라"가 안 먹은 것), 전부 `none`이면 게으른 것이다.
- **얼어 있던 가중치가 움직이나.** `notebooks/kospi_relations.py`의 궤적에서 `last_date`가
  오래된 요인이 없어지고, US10Y·WTI·KRBASE가 0 근처로 내려가는지(§4의 lead-lag 실측과 대조).
- **`unlisted_drivers`에 무엇이 쌓이나.** 같은 말이 셋 이상 반복되면 요인 후보다.

## 8.11 안 바뀐 값이 `none`으로 기록된다 — 2026-09-08 노동절 실측 (구현 2026-09-09, 미배포)

- 날짜: 2026-09-09(설계·구현, 워크트리 `feature-kospi-stale-observations`). 배포는 사용자가 한다 —
  머지, `just migrate upgrade head`(리비전 `e2a7c5d19b04`), pull.
- 여는 것: §8.10이 가르려던 "모른다(관측 없음)"와 "안다(무관, `none`)"가 값이 안 바뀐 날에
  다시 한 칸이 된다. 작은 것 둘(공시 화이트리스트, 열린 채 죽은 원장 행)을 같은 워크트리에서
  함께 고친다 — 셋 다 프롬프트를 안 건드려 판 8·관찰 판 3의 동결을 깨지 않는다.

### 왜 이 절이 있나

2026-09-07은 미국 노동절이었다. 09-08 장후 관찰이 받은 요인 값 표에서 SOX·S&P500·나스닥·
미국 10년물·VIX는 **9/4 종가 그대로**였다 — 전날 관찰이 이미 본 값이다. 프롬프트는 "값을
못 봤으면 `none`"이라 시키고(§8.10), 모델은 다섯 줄에 `none`을 적으며 노트에 "노동절로 신규
종가가 없다"고 남겼다. 그날 17줄 중 14줄이 `none`이었다.

`none`은 가중치 셈에서 분자 0·분모 정상이다(`relation_weight`). 그래서 휴장 하루가 이렇게
됐다(운영 `input_state.relations` 실측):

| 요인 | 09-08 장전 | 09-09 장전 | 09-08 관찰 |
| --- | --- | --- | --- |
| SOX | +0.89 (2관측) | +0.50 (3관측) | `none` — "노동절로 신규 종가가 없다" |
| VIX | −0.22 | −0.13 | `none` — 같은 이유 |
| S&P500 | +0.27 | +0.16 | `none` — 같은 이유 |

**"값이 안 바뀌었다"는 "봤는데 무관"이 아니라 "볼 것이 없다"다.** §8.10이 `none`을 만든 이유는
"봤더니 무관"을 0으로 세어 안 먹히는 요인을 내리려는 것이었는데, 볼 것이 없는 날의 `none`이
같은 0으로 들어가면 먹히는 요인도 휴장마다 내려간다. 미국 휴장은 해마다 아홉 번이고, 몇 달
안 바뀌는 기준금리는 매일 이 `none`을 받는다.

### 무엇을 한다 — 안 바뀐 줄은 표에서 뺀다

**모델이 판정하기 전에 코드가 거른다.** 요인 값의 날짜가 **지난번 관찰이 본 날짜와 같으면**
그 줄은 표에 싣지 않는다. 모델은 못 보고, 그래프에 엣지가 안 생기고, 원장에 `stale`로 센다.
가중치와 `n_obs`는 전날 그대로 넘어간다 — "모른다"가 "모른다"로 남는다.

```
19:00 장후 관찰
  코드가 요인 15개의 그날 값을 뽑는다                       (지금과 같다)
  ↓ 새 단계
  줄마다: 값 날짜 == 이 요인의 마지막 OBSERVED 엣지의 value_date ?
    같다   → 표에서 뺀다. observations_stale += 1. 엣지 없음
    다르다 → 표에 싣는다                                       (지금과 같다)
    값 없음(None) → 표에서 뺀다. stale로 센다 — 못 본 것은 판정할 수 없다
  ↓
  모델은 표에 있는 줄만 답한다                                (프롬프트 그대로)
  verify: 표에 있는 줄만 "다 답했나"를 묻는다                   (_tabled가 줄어든다)
  write_graph: 엣지에 value_date를 함께 쓴다                    (다음 날 비교의 기준)
```

"지난번 관찰이 본 날짜"의 원본은 **Neo4j `OBSERVED` 엣지의 `value_date`** 한 칸이다. Postgres에
따로 두지 않는다 — 관계의 원본은 그래프라는 결정(§1)과 같다. 첫날은 어제 엣지에 그 칸이
없어 전부 신선으로 본다. 그 뒤부터 비교가 선다.

**뉴스·공시 둘은 이 규칙 밖이다.** 값 표가 아니라 툴이고, 조회한 경우에만 관찰을 받는
규칙(§8.10)이 그대로다.

**프롬프트 60행("값을 못 봤으면 `none`")은 그대로 둔다.** 그 줄에 걸리는 경우가 코드에서
먼저 빠지므로 죽은 문장이 되지만, 문장을 지우면 판이 올라간다. 판 동결이 끝날 때 지운다.

### 데이터

| 무엇 | 어디 | 모양 |
| --- | --- | --- |
| 신선한 줄 / 안 바뀐 줄 | `domain.stale_factor_moves` | 순수 함수. `review.observe`가 표를 만들기 **전에** 가른다. `FactorMove`에 칸을 더하지 않는다 — 더하면 프롬프트에 실리는 표의 모양이 바뀐다 |
| 마지막 관찰이 본 값 날짜 | Neo4j `OBSERVED` 엣지 | `value_date` (date). `WRITE_OBSERVATIONS`가 쓰고 `READ_OBSERVATIONS`가 읽는다 |
| 읽은 관측 | `domain.Observation` · `RelationWeight.last_value_date` | `value_date: date \| None = None`. 프롬프트 행(`state.RelationRow`)에는 안 싣는다 |
| 뺀 줄 수 | `kospi_llm_run.observations_stale` | 정수, NULL 허용. 관찰 대화만. 수기 리비전 하나 |
| Slack 꼬리 | `render.py` | `none` 수 옆에 "값 안 바뀜 N" |

가중치 셈(`relation_weight`)은 안 바뀐다. `none`의 뜻도 안 바뀐다 — 봤더니 무관.

### 예시

09-08 관찰이 이 규칙으로 돌았다면:

```
요인 값 표 (모델에게 가는 것)                        빠진 줄 (원장 stale=5)
외국인 순매수   -2,459주    9/8                     SOX        9/4 = 어제 엣지 value_date 9/4
기관 순매수     +9,427주    9/8                     S&P500     9/4 = 어제 9/4
삼성전자        269,500원   9/8  -0.19%             나스닥     9/4 = 어제 9/4
SK하이닉스      ...                                 미국 10년물 9/4 = 어제 9/4
원달러          1344.83     9/7  -0.02%             VIX        9/4 = 어제 9/4
...             (12줄)
```

원달러·국고채·DXY·WTI는 9/7 값이지만 어제 엣지가 본 것은 9/4 값이라 **신선**이다. 하루 늦게
오는 값(ECOS·환율은 다음 날 아침 수집)은 이 규칙이 안 건드린다 — 어제 금리와 오늘 코스피를
짝짓는 셈인데 장전 전망이 실제로 쓰는 정보가 그것이라 그대로 둔다.

09-09 장전 관계 표는 SOX +0.89(2관측)로 어제와 같다. 09-08 실제 결과는 +0.50(3관측)이었다.

### 이미 쓰인 엣지 다섯

09-08의 SOX·S&P500·나스닥·미국 10년물·VIX `none` 엣지는 그래프에 남아 있다. **지우지 않는다.**
관측 창이 15개라 15거래일 뒤 스스로 밀려나고, 지우려면 운영 Neo4j에 컨테이너 안에서 Cypher를
쳐야 한다(§10의 4). 되돌릴 수 없는 삭제로 얻는 것이 보름이다.

### 함께 고치는 작은 것 둘

**공시 화이트리스트 둘.** `recent_disclosures`는 본문 있는 공시만 주는데(`body IS NOT NULL`),
2026-09-01~09 운영 실측에서 본문이 채워진 종류는 `최대주주등소유주식변동신고서` 하나뿐이었다
(4/4). 그 사이 SK하이닉스 풍문 해명(09-04), 한화에어로스페이스 풍문 해명(09-07), HMM
단일판매·공급계약(09-08)이 본문 없이 들어와 모델에게 안 보였다.
`collectors/document/dart.py`의 `MATERIAL_REPORT_KEYWORDS`에 `풍문또는보도`·`공급계약` 둘을
더한다. 이후 수집분부터 본문이 붙고 지난 건은 안 채운다. `조회공시`(거래소가 묻는 것)와
풍문 해명(회사가 스스로 내는 자율공시)은 이름이 달라 기존 조각에 안 걸렸다.

**열린 채 죽은 원장 행.** `kospi_llm_run` id 5(08-24 백필 1차 시도)가 `running`으로 영영
남았다. `finally`는 프로세스가 밖에서 죽으면 못 돈다. `start_llm_run`이 같은
`(kind, run_date, slot)`의 `running` 행을 먼저 `failed`·`error='orphaned by try N'`으로 닫고 새
행을 연다 — 같은 트랜잭션, SQL 하나(`kospi_llm_run/close_orphans.sql`). "돌다 죽었다"가
"아직 돌고 있다"로 남지 않는다. id 5는 다시 돌 실행이 없어 이 규칙이 안 닿는다 — 사용자가
운영 DB에서 한 번 닫는다.
...
| 무엇 | 어디 |
| --- | --- |
| `Observation.value_date`, `RelationWeight.last_value_date`, `stale_factor_moves` | `kospi/domain.py` |
| `OBSERVED.value_date`를 쓰고 읽는다. `ObservationWrite.value_date` | `kospi/graph.py` |
| `relation_rows_from` — 가중치를 한 번 읽어 표와 신선도 판정에 같이 쓴다 | `kospi/common.py` |
| 표를 만들기 전에 가르고, 뺀 줄을 원장·XCom에 센다 | `kospi/review.py` |
| Slack 꼬리 "값 안 바뀜 N: 이름" | `kospi/render.py` |
| `kospi_llm_run.observations_stale` | `apps/models/analysis/kospi.py`, 리비전 `e2a7c5d19b04`, `kospi_llm_run/finish.sql` |
| 고아 `running` 행 닫기 | `kospi_llm_run/close_orphans.sql`, `KospiStore.start_llm_run` |
| 화이트리스트 둘 | `collectors/document/dart.py` |

테스트 8개(도메인 3, 파이프라인 4, 공시 1)를 더했고 셋을 넓혔다(스키마·모델 SQL·원장 배선).
프롬프트 파일과 판은 안 건드렸다 — `test_prompt_versions.py`의 해시가 그대로다.

### 무엇을 보나

- `kospi_llm_run.observations_stale`가 미국 휴장 다음 날 5 근처, 평일 1~2(기준금리 등)인가.
- 휴장 다음 날 장전 관계 표에서 미국 지수 셋의 `weight`·`n_obs`가 전날과 같은가.
- 화이트리스트 뒤 `disclosure_event`에서 `풍문`·`공급계약` 행의 `body`가 채워지나.
...
6. **조용한 날의 관찰 커버리지.** 프롬프트의 "조용한 날은 관찰이 없어도 된다"가 "대충 해도
   된다"로 읽힌다. 2026-09-03 실측에서 15개 중 8개만 관측이 있었다. **§8.10이 닫는다** —
   모델이 요인을 고르지 않고 코드가 15개를 다 준다. 다음 판.
