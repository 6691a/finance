...
# 6: 산문의 숫자 표기 규칙(`llm.NUMBER_STYLE`)이 붙었다(2026-08-26). 천 단위 쉼표와 단위를
#    요구한다. 그전에는 "상승 1174", "외국인이 140762백만원"처럼 툴 JSON의 raw 숫자가
#    그대로 문장에 실렸다.
PROMPT_VERSION = "6"
...
# 답변 단계에 쓸 자리가 없다. 호출 상한과 함께 올린다(2026-08-25에 24,000에서). 한 호출이
# 20건 × 600자를 다 쓰면 12,000자라, 이 값은 그런 호출 서넛을 담는 크기다.
