...
`index_bar/select_horizon_return.sql`과 `select_intraday_horizon_return.sql`이 지수 **분봉**의
마감 봉을 읽는다. 그래도 T+5까지라 정규 수집이 덮는 범위 안이고, 그보다 과거를 보는 곳은
없다.
...
**매물대를 계산할 때 두 거래소를 합치면 안 되는 이유가 이 숫자에 있다** — 같은 종목이라도
체결이 갈리고 날짜 집합부터 다르다.

#### 000660 NXT의 열한 날 구멍 — 필터가 막은 사고

`000660` NXT만 240일이고 `2025-12-11 ~ 12-26`(11거래일)이 비어 있다. 같은 날 같은 run이
`005930` NXT는 690봉, `000660` KRX는 381봉을 정상으로 받았다. 제공처에 물어 갈랐다
(2026-08-27, 조회 전용).

```
000660 NXT 2025-12-15 요청 → rt_cd=0, 120봉, 전부 날짜 20251210
000660 NXT 2025-12-18 요청 → rt_cd=0, 120봉, 전부 날짜 20251210
000660 NXT 2025-12-29 요청 → rt_cd=0, 120봉, 날짜 20251229   (재개)
005930 NXT 2025-12-15 요청 → rt_cd=0, 120봉, 날짜 20251215   (같은 날 정상)
```

**KIS는 요청한 날짜에 데이터가 없으면 마지막으로 있던 세션의 봉을 정상 응답으로 돌려준다.**
그 종목이 NXT에서 그 2주간 체결되지 않았고, `fetch_stock_bars`가 요청 날짜로 걸러 0봉으로
넘긴 것이다. 수집은 정상이었다.

**이 구멍은 §3.3의 날짜 필터가 제 일을 한 증거다.** 필터가 없었으면 12-10 봉이 열한 개
날짜에 복제돼 들어가고, 그 값으로 그린 매물대와 지지선이 조용히 틀렸을 것이다. 빈 날짜는
빈 채로 남기는 편이 맞다.
...
### 다른 분봉은 어디까지 되나 (2026-08-27 실측)

같은 질문을 나머지 계열에도 물었다. **제공처마다 답이 다르고, 지수는 아예 안 된다.**

| 계열 | 백필 | 근거 |
| --- | --- | --- |
| 국내 종목 (KRX·NXT) | **1년** | `FID_INPUT_HOUR_1`이 진짜 커서다. 위에서 채웠다 |
| 국내 지수 (KOSPI·KOSDAQ·KOSPI200) | **불가** | 아래 |
| Yahoo 전부 (환율·상품·해외지수·선물·코인) | **최대 30일** | `yahoo_quote_intraday`에 `backfill_start`·`backfill_end`가 이미 있고 `BAR_RETENTION_DAYS = 30` 가드가 막는다 |
| 국내 지수선물 | 가능해 보임 | `fetch_bars(..., until)`이 커서를 받는다. 과거 구간은 그때의 월물 코드가 필요해 미확인 |

**국내 지수 과거 분봉은 KIS가 주지 않는다.** 공식 예제로 확인한 것이 셋이다.

- `inquire-daily-indexchartprice`(FHKUP03500100) — 날짜 구간을 받지만 `FID_PERIOD_DIV_CODE`가
  `D`·`W`·`M`·`Y`뿐이다. **분봉이 없다.**
- `inquire-index-timeprice`(FHPUP02110200) — 분 단위지만 날짜·기준시각 파라미터가 없다. 당일뿐.
- `inquire-time-indexchartprice`(FHKUP03500200) — 우리가 쓰는 것. 역시 당일뿐이고 그
  `FID_INPUT_HOUR_1`은 기준 시각이 아니라 봉 간격이다(§3.3).

**지수가 종목의 합이니 종목이 되면 지수도 되어야 하지 않나**는 질문의 답은 "합을 계산해
보관하는 주체가 다르다"이다. 종목 체결은 거래소 원장에 남아 증권사가 그날 그 시각 봉을
재구성해 주지만, 지수는 KRX가 실시간 산출해 뿌리는 파생 지표이고 오픈API는 그 산출값을
당일 화면용으로 중계할 뿐이다. 직접 계산하려면 구성종목 전체의 분봉과 상장주식수·유동비율·
기준시가총액이 있어야 하는데, 코스피200으로 좁혀도 하루 3,200콜이라 현실적이지 않다.

**그래서 지수는 일봉으로 판단한다.** 애초에 그쪽이 맞는 해상도다 — "이 값을 지지선으로
횡보했다"는 몇 달~몇 년짜리 이야기라 분봉으로 보면 노이즈만 는다. `index_daily`가
2016-08부터 있다. 분봉이 값어치를 갖는 자리는 종목의 정밀 매물대이고 그건 위에서 1년을
확보했다.

| | 다년 구조 (지지·저항, 사건 매핑) | 분 단위 정밀 (매물대) |
| --- | --- | --- |
| 지수 | `index_daily` 2016-08~ | 오늘부터 쌓이는 것만 |
| 종목 | `stock_investor_trade_daily` 2018-12~ | `stock_bar` 1년 |

**분봉 백필 설계 문서가 따로 생기면 그 문서가 이 절을 대체한다.**
