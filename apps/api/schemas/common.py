...
- **확률·등락률·점수는 JSON number다.** `Decimal`을 그대로 두면 Pydantic이 문자열로
  직렬화해 클라이언트가 매번 파싱한다. Neo4j로 보낼 때도
  `float`로 바꾸기로 이미 정했고, 유효자리가 `Numeric(5,4)`·`Numeric(8,4)`라
...
def _serialize_utc(value: datetime) -> str:
    ...
    이 함수는 `None`을 안 받는다 — 애노테이션이 `UtcDatetime | None`이어도 Pydantic은
    datetime 쪽 가지에서만 부른다.
    """
