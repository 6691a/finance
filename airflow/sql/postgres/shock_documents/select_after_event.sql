...
-- 옛 판단을 다시 만들지 않게 한다.
--
-- **점수 하한은 정렬이 아니라 술어다.** `ORDER BY ... LIMIT`만으로는 후보가 상한보다
-- 적을 때 아무것도 안 잘려 부고·인사·행사 기사가 그대로 모델에 간다. 사건 이후 창은
-- 1차 시도에서 특히 좁아 그때가 제일 취약하다. 값은 코드 상수(`MIN_DOCUMENT_SCORE`)이고
-- 모델이 고르는 인자가 아니다.
SELECT document.id,
