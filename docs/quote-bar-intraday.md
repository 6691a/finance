...
> | `compose/local/grafana/dashboards/quote-*.json` | 대시보드 여섯 (종류별 다섯 + 통합) |
> | `tests/dashboards/test_quote_dashboards.py` | 대시보드끼리·모델과 대조 |
>
> 검증 결과는 §7, 백필은 §8, 대시보드는 §9, 국내 수집은 §10, 심볼 확장은 §11에 있다.
> 구간 선택은 §13에 있다. §6은 설계 단계에서 확인한 저장소·도구
> 관련 사실들이다.
>
...
CME가 싸게 나오면 미국 **선물**을 KIS로 옮기는 걸 재검토한다. 테이블·SQL·대시보드는 어느
쪽이든 그대로 쓰인다. `provider` 컬럼이 그 교체를 흡수한다.

#### 프로브 F — 해외지수 현재가는 지연이 아니다 (2026-08-10 실측)

F 를 처음 돌렸을 때는 "60초 뒤 값이 바뀐다"까지만 봤다. **그건 답이 아니었다.** 15분 지연
시세도 60초마다 갱신된다. 그래서 KIS `SPX` 현재가와 Yahoo `^GSPC`를 같은 순간 맞댔다.

| EDT | KIS `ovrs_nmix_prpr` | Yahoo `regularMarketPrice` | 차이 |
| --- | ---: | ---: | ---: |
| 10:12:40 | 7764.16 | 7764.17 | −0.01 |
| 12:13:53 | 7761.91 | 7761.86 | +0.05 |
| 13:14:59 | 7754.08 | 7754.09 | −0.01 |
| 15:34:15 | 7750.65 | 7750.70 | −0.05 |
| 16:35:36 (마감 후) | 7753.11 | 7753.11 | 0.00 |

**소수점 둘째 자리까지 같다.** 15분 지연이면 SPX가 그 사이 최소 수 포인트는 움직인다.
남은 차이는 두 호출 사이의 몇 초다(한 샘플의 +1.69도 Yahoo 기준시각이 2초 전이었다).
마감 뒤에는 둘 다 같은 종가에 고정된다.

**그래도 미국은 Yahoo 를 유지한다.** 옮길 수 있다는 것과 옮길 이유가 있다는 건 다르다.

- 한국 장중에 살아 있는 미국 값은 **선물**이다(§12.1). 그건 CME 시세료에 걸려 여전히 막혀 있다.
- KIS 해외지수는 **분봉이 장중에 갱신되지 않는다**(§2.1). 옮기면 현재가를 1분 폴링해 봉을
  직접 만들어야 한다. Yahoo 는 봉을 통째로 준다.
- 지수(SPX·SOX·VIX)는 한국 정규장에 0봉인 기록용이다. 소스를 바꿔도 그 성질은 안 변한다.

**대신 백업 소스로서는 값어치가 있다.** Yahoo 는 백오프가 없고 차단 위험이 있는데(§12.6),
KIS 해외지수는 무료이고 실시간이다. Yahoo 가 막히면 SPX·SOX·VIX·NDX·COMP 를 여기서 받을 수
있다는 뜻이다. 지금 붙이지는 않는다.
...
| 선택한 구간만 | `session`이 고른 구간 봉만 남기고 구간 시작을 0으로 재정규화 |
| 수집 상태 | 심볼별 마지막 봉과 지연 |

변수는 `ds`, `provider`, `symbol`, `session`, `window_min`, `threshold`다.
...
그래서 **이름을 고치려면 `quote_symbol` 한 행만 바꾸면 된다.** 대시보드 여섯 개를 건드리지
...
전수 조사가 실패한 건 유량 제한 탓도 있지만(`EGW00201`, §12.5에서 실측) **형식 자체가
틀렸기 때문**이다. 답은 KIS 종목 마스터 파일에 있었다.
...
## 12. 25종에서 30종으로 (2026-08-09)

§11이 6종 → 25종 확장의 근거를 남겼다면 여기는 **그 다음**이다. 후보를 전부 실제 API로
확인한 뒤 **1순위 다섯 종을 붙였다.** 2·3순위는 아직이고, 나중에 구현할 때 다시
프로브하지 않아도 되게 응답 필드까지 그대로 적어 둔다.
...
**1순위 — 붙였다.** Enum 한 줄 + 마스터 시드 한 줄로 끝났다.
...
암호화폐만 `kind`가 없어서 `crypto`를 여덟 번째로 열었다(`f7c089e70905`). 나머지 셋은
기존 `index_future`라 시드 한 줄이 전부다. 대시보드도 `quote-index-future`가 `kind`로
고르기 때문에 손대지 않았고, `crypto`만 새 화면 하나를 만들었다.

**2순위 — 아직. 국내 반도체 개별주(삼성전자·SK하이닉스).**
...
**3순위 — 아직.** `ZT=F`·`ZF=F`(장단기 금리차를 한국 장중에 본다. 지금은 10년만 있다), `NG=F`.
...
| 대상 | 과거 조회 | 한 요청 | 하루치 |
| --- | --- | ---: | --- |
| 선물 | **된다** — `FID_INPUT_DATE_1`+`FID_INPUT_HOUR_1` | 102봉 | 408봉 / **요청 4회** |
| 주식 | **된다** — 별도 TR `FHKST03010230` (`inquire-time-dailychartprice`) | 120봉 | 381봉 / **요청 4회** |
| 업종지수 | **안 된다** | 120봉 | 이 엔드포인트의 `FID_INPUT_HOUR_1`은 기준 시각이 아니라 봉 간격이다(§10.4) |

즉 지수만 앞으로 쌓이는 것을 기다려야 하고 선물과 주식은 과거를 채울 수 있다.
`kis_quote_intraday`가 지금 그 인자를 쓰지 않을 뿐이다.

#### 얼마나 뒤까지 가는가 — 실측 (2026-08-10)

**선물과 주식의 보존 기간이 다르다.**

| 조회 시점 | 선물 (`A0`+상품+월물) | 주식 (`005930`) |
| --- | --- | --- |
| 90일 전 | 102봉 | 120봉 |
| **180일 전** | **102봉** | 120봉 |
| **365일 전** | **0봉** | **120봉** |
...
선물은 **약 6개월**, 주식은 **약 1년**이다(정확한 경계는 180~365일 사이, 365~730일 사이).

선물의 365일 0봉은 **계약이 없어서가 아니다.** 그날의 최근월물(`A01509`)로 쳐도 0봉이다.
제공처의 보존 한계다.

**백필은 그날의 최근월물로 쳐야 한다.** 지금 상장된 `A01609`로 과거를 조회하면 응답이
오긴 하는데 102봉이 `20251119`~`20260209`에 흩어져 나온다. 그 시점에 원월물이라 거의
거래되지 않았기 때문이다. `front_contract(future, that_day)`가 이미 날짜를 인자로 받으므로
백필 루프가 하루마다 다시 계산하면 된다.

#### 유량 제한 — 추측이 아니라 실측이다

§10.2가 전수조사 실패를 "유량 제한으로 보임"이라고 적었는데 **코드까지 확인했다.**

- 초과하면 **HTTP 500 + `EGW00201` "초당 거래건수를 초과하였습니다"** 다.
- 쉬지 않고 30회를 던지면(9.1 req/s) **15회가 실패한다.** 0.2초 간격(5 req/s)에서도 산발
  실패한다. **1.0초 간격에 재시도 2회**를 붙이니 100% 통과했다.
- 500이라 `kis_quote_intraday`의 `UNRECOVERABLE_STATUSES`(400·403·404)에 걸리지 않고
  그대로 재시도된다. 폴링은 5분에 5요청이라 애초에 닿지 않는다. **백필만 이 페이싱이 필요하다.**

초당 1건이면 선물 2종 6개월(≈124영업일 × 4회 × 2) 약 1,000요청에 17분, 주식 2종
1년(≈250영업일 × 4회 × 2) 약 2,000요청에 33분이다. 한 번 돌리고 끝나는 규모다.

**토큰은 짧은 간격에 세 번 받으면 403이다**(실측). 백필 스크립트도 토큰을 한 번만 받아
재사용해야 한다.

주식 TR `FHKST03010230`은 `FID_FAKE_TICK_INCU_YN`을 **필수로 받는다.** 빼면 `rt_cd=2`
`INPUT FIELD NOT FOUND`다. 분봉 TR(`FHKST03010200`)과 다르다.
...
### 12.7 1순위 구현 — 무엇이 실제로 바뀌었나

| 심볼 | 제공처 | `kind` | 확인 |
| --- | --- | --- | --- |
| `KOSDAQ150_FUT` | kis | `index_future` | `A06609`로 월물이 풀리고 금요일 구간 61봉 파싱 |
| `RUSSELL2000_FUT` | yahoo | `index_future` | 폴링에서 정상 수집 |
| `DOW_FUT` | yahoo | `index_future` | 폴링에서 정상 수집 |
| `BTC` · `ETH` | yahoo | `crypto` | 일요일 11:24 KST에 각 120봉 |

**바꾼 곳은 여섯이다.** 수집기 Enum 둘(`yahoo.py`·`kis.py`), 모델의 `QuoteSymbolKind`,
마이그레이션 하나, 대시보드 하나, 테스트 둘. `quote_bar`도 DAG도 손대지 않았다.
KIS DAG는 `DomesticFuture`를 순회하므로 `KOSDAQ150_FUT`가 자동으로 붙었고,
`quote-index-future` 대시보드는 `kind`로 고르므로 새 선물 셋이 저절로 들어왔다.
**§9.2가 마스터 테이블을 둔 이유가 여기서 다시 확인된다.**

**주말 공백이 실제로 그렇다는 증거를 얻었다.** 일요일 11:24 KST에 Yahoo DAG를 돌리니
25종 중 `BTC`·`ETH` 둘만 봉을 냈다.

```
symbol | 봉  |      처음_kst       |     마지막_kst
--------+-----+---------------------+---------------------
 BTC    | 120 | 2026-08-09 09:25:00 | 2026-08-09 11:24:56
 ETH    | 120 | 2026-08-09 09:25:00 | 2026-08-09 11:24:38
```

나머지 23종은 최근 3시간 동안 0봉이다. §12.1의 구멍 2를 사후가 아니라 **수집 중에**
확인한 셈이다.

#### 주말 구간을 보는 패널

`crypto` 대시보드를 만들면서 "한국 정규장 시간대만" 패널을
**"주말 구간만 (토 07:00 ~ 월 07:00 KST)"** 으로 바꿨다. 곧 §13이 이걸 일반화해서
`session` 변수의 `weekend` 값으로 대체했고, 그래서 지금은 여섯 대시보드가 같은 패널을 쓴다.

주말 경계는 KST에서 7시간을 당겨 접는다. 그러면 토 07:00~월 07:00이 토·일 두 날짜로
접히고, 그 주의 토요일 날짜가 **주말 하나의 식별자**가 된다.

```sql
EXTRACT(ISODOW FROM (bar_at AT TIME ZONE 'Asia/Seoul') - interval '7 hours') IN (6, 7)
(date_trunc('week', (bar_at AT TIME ZONE 'Asia/Seoul') - interval '7 hours')
   + interval '5 days')::date AS weekend_id
```

경계를 실제로 확인했다. 토 06:59는 빠지고 07:00부터 들어가며, 월 06:59까지 같은
`weekend_id`(그 주 토요일)로 묶이고 07:00부터 다음 주말로 넘어간다.

#### 국가가 없는 자산

`quote_symbol.country`는 NOT NULL인데 비트코인에는 국가가 없다. ISO 3166-1이 사용자
할당용으로 비워 둔 `XA`~`XZ`에서 `XX`를 쓰고 표시 이름은 "글로벌"로 뒀다.
`indicator_series`가 유로존에 `XM`을 쓰는 것과 같은 규칙이다.

#### `front_contract`가 상품을 안 가린다

`KOSDAQ150_FUT`를 붙이면서 이 함수가 처음으로 심볼 둘을 받게 됐다. 분기물이라는 전제가
함수 하나에 박혀 있어서, **월물 상품을 넣으면 조용히 틀린 계약을 조회한다.**
0봉으로 끝나므로 눈에 띄지도 않는다. `test_every_future_shares_the_quarterly_roll`이
모든 `DomesticFuture`가 분기월로 떨어지는지 확인한다. 월물 상품을 넣으려면 그때
`CONTRACT_MONTHS`를 Enum 속성으로 내려야 한다.

---

## 13. 구간을 대시보드가 아니라 변수로 고른다 (2026-08-09)

§9.4의 "한국 정규장 시간대만" 패널은 구간이 **하나로 박혀 있었다.** 미국 정규장만 보거나
시간외만 보려면 패널을 더 만들거나 대시보드를 또 나눠야 했다. `session` 변수 하나로 바꿨다.

| 값 | 뜻 |
| --- | --- |
| `all` | 전체 (기본) |
| `kr` | 한국 정규장 — 서울 09:00~15:30, 평일 |
| `us` | 미국 정규장 — 뉴욕 09:30~16:00, 평일 |
| `off` | 시간외 — 두 정규장 **어디에도 안 드는** 시간 |
| `weekend` | 주말 — 토 07:00 ~ 월 07:00 KST |

Grafana 12에 라디오 버튼형 변수는 없다. 변수는 전부 드롭다운으로 뜬다.

### 13.1 경계는 벽시계가 아니라 현지 시각으로 판정한다

미국 정규장은 KST로 22:30~05:00이었다가 서머타임이 끝나면 23:30~06:00이 된다.
**KST 시각을 상수로 쓰면 1년에 두 번 조용히 틀린다.** 그래서 각 시장의 현지 시각으로 본다.

```sql
(bar_at AT TIME ZONE 'America/New_York')::time BETWEEN '09:30' AND '16:00'
AND EXTRACT(ISODOW FROM bar_at AT TIME ZONE 'America/New_York') BETWEEN 1 AND 5
```

IANA 시간대 데이터가 DST를 처리하므로 전환일에도 맞는다. CLAUDE.md의 "DST는 고정 offset이
아니라 IANA 데이터로 처리한다"를 그대로 따른 것이다.

`off`(시간외)는 정규장 목록의 **여집합**으로 정의했다. 구간을 나열해 정의하면 나중에
시장을 추가할 때마다 여기도 함께 고쳐야 하고, 빠뜨리면 어느 쪽에도 안 잡히는 봉이 생긴다.

### 13.2 술어를 어디에 꽂는가

창 함수(`RANGE ... PRECEDING`)를 계산하기 **전에** 거르면 구간 첫 봉의 변화율이 0이 된다.
09:00 봉의 15분 창에 08:45~08:59가 없어지기 때문이다.

보고 싶은 것은 "이 구간에 일어난 급변"이지 "이 구간만으로 다시 센 변화율"이 아니다.
그래서 변화율을 다 계산한 **뒤** 마지막 단계에서 거른다. 급변 이벤트 표와 최근 N분
변화율 둘 다 그렇다.

직전 거래일 대비 변동률 패널은 또 다르다. 기준일 종가를 잡으려면 조회 구간보다 앞을 더
읽어야 하므로 **기준선 계산에는 필터를 걸지 않고** 최종 SELECT에서만 건다.

### 13.3 검증 — 구간이 진짜로 분할인지

원자재 대시보드의 다섯 패널 × 다섯 구간, 서른 조합을 실제 DB에 던졌다. 전부 실행되고,
봉 수가 정확히 나뉜다.

```
전체 2,518 = 한국 정규장 780 + 미국 정규장 782 + 시간외 956
```

겹치지도 새지도 않는다. 암호화폐 대시보드에서는 같은 쿼리가 이렇게 나온다.

```
전체 313 = 주말 313,  한국 정규장 0,  미국 정규장 0
```

지금까지 모은 암호화폐 봉이 **전부 주말 구간**이라는 뜻이다. 1순위로 붙인 지 하루가
안 됐고 그 하루가 주말이었다. §12.1의 구멍 2가 다시 확인된다.

### 13.4 대시보드는 나눈 채로 둔다 — 합쳐 봤다가 되돌렸다

`session`을 넣고 나서 **`kind`도 같은 방식으로 고르게 하고 다섯을 지웠다.** 하루도 안 돼
되돌렸다. 되돌린 이유를 남긴다.

#### 왜 되돌렸나 — 임계값이 종류마다 18배 다르다

15분 변화율 상위 1%를 종류별로 재 봤다.

| 종류 | 상위 5% | 상위 1% | 임계값 1.0%로 잡히는 분 | 전체 분 |
| --- | ---: | ---: | ---: | ---: |
| 지수 | 1.704% | **3.081%** | 3,046 (14%) | 21,686 |
| 원자재 | 0.482% | 0.966% | 45 | 4,981 |
| 지수선물 | 0.259% | 0.464% | 10 | 46,102 |
| 국채선물 | 0.058% | 0.327% | **0** | 1,129 |
| 환율 | 0.099% | 0.236% | **0** | 5,490 |
| 암호화폐 | 0.104% | **0.166%** | **0** | 665 |

합친 대시보드는 임계값 기본값이 하나(1.0%)다. 그 상태에서 종류를 `환율`로 바꾸면
**급변 이벤트 표가 텅 빈다.** 오류도 안 나고 화면도 멀쩡하다. "환율은 급변한 적이 없다"로
읽힌다. 반대로 `지수`는 전체 분의 14%가 걸려 "급변"이라는 말이 의미를 잃는다.

**이 오류는 테스트로 못 잡는다.** 사람이 드롭다운을 어떻게 조합하느냐에 달렸기 때문이다.
나눠 두면 지수 화면은 1.0%, 환율 화면은 0.2%가 처음부터 박혀 있어서 **잘못된 조합을 만들
수가 없다.**

축 문제도 같다. 합치면 암호화폐(±5%)와 원자재(±0.5%)를 한 축에 올릴 수 있고, 그러면
원자재 선이 평평해 보인다. 나눠 두면 그 상태가 아예 표현되지 않는다.

#### 합침이 주는 이점은 테스트로 대신할 수 있다

합쳐서 얻는 건 "같은 패널 SQL 여섯 벌이 서로 어긋나지 않는다"뿐이다. 실제로 `session`을
넣을 때 여섯 파일에 같은 SQL을 주입해야 했다. 그런데 **이건 테스트로 잡힌다.**

`tests/dashboards/test_quote_dashboards.py`가 `kind` 조건과 통합 전용 "종류" 열을 걷어낸 뒤
여섯 벌의 패널 SQL이 글자 그대로 같은지 본다. 달라도 되는 자리를 명시적으로 적어 뒀으므로
그 밖의 차이는 전부 실패다. 환율 대시보드의 이벤트 묶는 간격만 10분에서 20분으로 바꿔
실제로 걸리는지 확인했다.

정리하면 이렇다. **읽는 시점의 착각은 구조로 막고, 유지보수 시점의 드리프트는 테스트로
막는다.** 앞은 테스트로 못 막고 뒤는 구조가 필요 없다.

#### 이때 잡은 버그 — 암호화폐 임계값

`quote-crypto.json`을 `quote-commodity.json`에서 복사해 만들면서 임계값 기본값 0.5를
그대로 물려받았다. 암호화폐의 상위 1%가 0.166%라 **급변 이벤트가 영원히 0건**이었다.
0.2로 고쳤다.

측정하지 않았으면 못 찾았을 버그다. 빈 표는 오류가 아니라 "그런 일이 없었다"로 보인다.
같은 실수를 막으려고 `test_threshold_default_is_in_range_for_its_kind`가 기본값이 그 종류의
실측 상위 1%와 같은 자릿수인지(1/5배 ~ 3배) 확인한다.

암호화폐 표본은 아직 665분뿐이고 그게 전부 주말이라, 평일 데이터가 쌓이면 이 값은 다시 봐야 한다.

#### `kind` 지수 값이 큰 것은 VIX 때문이다

지수의 상위 1%가 3.081%로 유독 큰데 심볼별로 보면 VIX가 3.630%로 혼자 끌어올린 것이다.
나머지 지수는 0.270%(항셍) ~ 0.853%(닛케이)다. 그래서 지수 화면의 1.0%는 코스피·닛케이에는
맞고 VIX에는 낮다. VIX가 자주 걸리는 건 그 자체가 신호라 그대로 뒀다.

**임계값은 사실 `kind`가 아니라 심볼 단위여야 한다**(§9.6). 종류별 기본값은 그 근사치다.
더 정확히 하려면 급변 판정을 상수 대신 **심볼별 분위수**로 바꾸면 된다. 그러면 종류를
섞어도 각자 자기 분포로 판단한다. 다만 정의상 항상 상위 N%가 걸리므로 절대 하한을 함께
걸어야 하고, 그건 아직 하지 않았다.
