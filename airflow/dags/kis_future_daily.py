...
`start_date`를 주면 그 날짜까지 거슬러 올라간다. 만기된 계약도 과거 날짜로 조회되지만
**영원히는 아니다.** KIS는 만기된 월물을 얼마 뒤 목록에서 지우고, 지워진 코드에는
`rt_cd=0`·`msg1="정상처리 되었습니다"`에 빈 `output2`로 답한다. 오류가 아니라 조용한 0건이다.

2026-08-28 실측에서 살아 있던 것은 최근 분기물 셋뿐이다.

| 월물 | 만기 | 만기 후 | 그 월물 창 |
| --- | --- | --- | --- |
| `A01609`·`A06609` | 2026-09-10 | — | 받힌다 |
| `A01606`·`A06606` | 2026-06-11 | 2.5개월 | 받힌다 |
| `A01603`·`A06603` | 2026-03-12 | 5.5개월 | 받힌다 |
| `A01512`·`A06512` | 2025-12-11 | 8.6개월 | **0행** |
| `A06509` | 2025-09-11 | 11.6개월 | **0행** |

`A01512`는 조회 창을 20240101~20251231로 넓혀도 0행이다. 창이 좁아서가 아니라 계약이 없다.

그래서 **백필 하한은 그때 살아 있는 가장 오래된 월물의 창 첫날이다.** 2026-08-28 기준
`2025-12-12`(`A01603` 창의 첫날, 직전 만기 2025-12-11의 다음날)이고, 이 값은 만기가 지날 때마다
앞으로 밀린다. 하한보다 이른 `start_date`를 주면 첫 200달력일 창이 통째로 비어 그 심볼이
`KisPayloadError`로 죽는다 — 부분 저장 없이 실패한다.
...
**받을 수 있을 때 받아 둔다.** 이 DAG가 매일 도는 한 시계열은 앞으로 이어지지만, 하한보다
과거는 KIS에서 다시 받을 방법이 없다. 위 백필로 심볼당 172행(2025-12-12~2026-08-27)이 들어왔다.

**사용자가 계약 코드를 직접 넣는 파라미터는 두지 않는다.** 논리 시계열의 롤 규칙을 우회할
통로가 되고, 그렇게 들어간 봉은 나중에 어느 규칙으로 모였는지 알 수 없다.

월물 코드는 연도를 한 자리로 담아(`A01609`) 10년마다 겹치지만, 그 충돌은 실무에서 드러나지
않는다. 보관 한계가 훨씬 먼저 오기 때문이다 — 겹치는 상대인 10년 전 계약은 이미 없다.
