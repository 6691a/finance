...
대조한다. 설계 문서는 `docs/kis-market-positioning-daily.md`다.
...
def store_credit_balance(connection: Connection, fetch: Fetch) -> int:
...
def store_credit_ranking(connection: Connection, fetch: Fetch) -> int:
...
def store_market_funds(connection: Connection, fetch: Fetch) -> int:
    with connection.cursor() as cursor:
        source_record_id = _insert_source_record(cursor, fetch, len(fetch.rows))
        execute_upserts(
            cursor,
...
def store_short_sale(connection: Connection, fetch: Fetch) -> int:
    with connection.cursor() as cursor:
        source_record_id = _insert_source_record(cursor, fetch, len(fetch.rows))
        execute_upserts(
            cursor,
...
def store_market_lending(connection: Connection, fetch: Fetch) -> int:
...
def store_lending(connection: Connection, fetch: Fetch) -> int:
    with connection.cursor() as cursor:
        source_record_id = _insert_source_record(cursor, fetch, len(fetch.rows))
        execute_upserts(
            cursor,
