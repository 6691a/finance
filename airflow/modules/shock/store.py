...
class ShockStore:
    ...
    def documents_after(self, *, event_at: datetime, as_of_at: datetime, limit: int) -> list[DocumentRow]:
        """급변 시각 **이후** 발행된 문서. 그 이전 것은 아예 안 준다."""
        with self._connection.cursor() as cursor:
            cursor.execute(
                SELECT_DOCUMENTS_AFTER,
                {"event_at": event_at, "as_of_at": as_of_at, "limit": limit},
            )
            rows = cursor.fetchall()
        return [
            DocumentRow(
