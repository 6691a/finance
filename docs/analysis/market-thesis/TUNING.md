...
- 대상: 시장 추론 전 단계 (`market_thesis_forecast`·`market_thesis_review` DAG와 `airflow/modules/thesis/`)
- 성격: 단계 문서가 아니다. 1~6이 "무엇을 만드나"의 순서라면 이 문서는 **만든 뒤에 쓰는
  운영 규칙**이다. [README.md](README.md)와 같은 층이다.
- 상태: **캘린더의 0일이 시작됐다.** 3-dag-slack.md 7절의 선행 조건 둘은 해소됐다 —
  리비전은 운영 head까지 반영됐고(2026-08-28 실측) 추론도 돌고 있다(`thesis` 82행,
  프롬프트 판 1~12가 섞여 있다). 실제 채점 결과는 아래 6절에 있다.
...
| 장중 스케줄 | 12:35 KST 하나 | `thesis.state.INTRADAY_SLOT_TIMES` + `market_thesis_intraday.SCHEDULE` — **두 곳** | 같은 것 + 슬롯별 발행률 | 문서 평가(:25) 뒤라 :35다. **2026-08-28에 넷(10:35·12:35·14:35·15:00)에서 `intraday_midday` 하나로 줄였다** — 근거는 6절. **두 곳을 같은 커밋에서 만진다** — 테스트가 대조하고, 어긋나면 `resolve_slot`이 실행을 죽인다. 슬롯 라벨은 표에서 만들어져 따라온다 |
...
| `x-grok-conv-id` | 실행마다 하나 (`thesis-{run_date}-{slot}`) | `llm.py` `_conversation_headers`, 부르는 곳 넷 | LangSmith의 `prompt_token_details.cache_read` ÷ `prompt_tokens`. **원장은 이 값을 안 센다**(13-llm-ledger.md "토큰 셋") | **xAI 캐시는 서버마다다.** 헤더가 없으면 왕복마다 다른 서버로 가 대화 전체를 새로 낸다 — 2026-08-27 실측에서 적중이 21.5%(246,395 중 52,992)였다. 값은 **결정적**이어야 한다(난수면 재시도가 캐시를 버린다). 적중률이 안 오르면 값이 실행마다 갈리고 있는지부터 본다 |
| 프롬프트 JSON 들여쓰기 | 없음 (`prompt.json_dump`) | `modules/prompt.py` | `thesis_llm_run.prompt_tokens`의 슬롯별 추이 | 들여쓰기 공백은 그대로 입력 토큰이고 **툴 왕복마다 재전송**된다. 2026-08-27 실측: 관측 상태 블록 15,144자 → 6,744자(-55%), 세 블록 합 -14,450자, 네 왕복이라 약 32,000 토큰. **되돌릴 이유가 생기기 어렵다** — 모델은 들여쓰기 없는 JSON을 똑같이 읽고 사람이 볼 때는 LangSmith가 포매팅한다 |
...
| `BUILD_TIMEOUT` | 30분 | `thesis/common.py` | `build_thesis`의 `AirflowTaskTimeout` 건수와 성공 실행의 소요 분포 | 요청 타임아웃의 바깥 울타리. 한 빌드는 모델을 최대 왕복 5 + 답변 + 교정 = 8번 부른다(`MAX_TOOL_ROUNDS`가 2026-08-27에 3 → 5). 장전이 09:00 개장 전에 닿아야 해서 이 값이고, 걸리면 `MAX_TOOL_ROUNDS`를 먼저 의심한다. 재시도 셋은 그대로라 최악 4회 × (30 + 10)분이다 |
...
| `MEET_BAND_PCT` | `5.0` | `expectation/domain.py` | `stock_event_outcome.verdict` 분포 | 기대 대비 발표를 `meet`로 볼 폭(퍼센트). **실측이 아니라 시작값이다** — `meet`가 사실상 없거나 대부분이면 그 폭이 틀린 것이다. `FLAT_THRESHOLD_PCT`와 같은 성격이고 판단 시점도 같다(+4주) |
| 추출 `PROMPT_VERSION` | `"1"` | `expectation/domain.py` | 버림 로그의 사유 분포 | 프롬프트를 고치면 올린다. **올리면 이미 뽑은 문서가 전부 재추출 대상이 된다** — `document.prompt_version`과 같은 장치다. 버림 사유가 한 유형에 몰리면 프롬프트 예시를 보강할지 `StockEventType`을 늘릴지를 그 로그가 정한다 |
| 추출 `DEFAULT_BATCH_SIZE` | 50 | `expectation/domain.py` | 대상 문서 백로그 | 대상이 종목 태그 문서뿐이라 지금 물량에서는 남는다. watched가 크게 늘면 이 값과 대상 조건(`value_score` 하한 추가)이 손잡이다 |
...
| 날짜 | 손잡이 | 변경 | 본 숫자 | 근거 |
| --- | --- | --- | --- | --- |
| 2026-08-27 | 부분 답 처리 | 그대로 저장 → **한 번 다시 묻는다**(`PROMPT_VERSION` 9) | `intraday_midday`가 대상 넷을 조사(daily_history 4회)해 놓고 **하나만 답했다.** 툴 16호출 전부 성공, 검증이 버린 것 0, 교정 안 돎 — 모델이 답변 한 번에 하나만 냈다 | 그 실행이 `written=1`로 성공이었고 Slack에 KOSPI 한 줄만 나갔다. 교정본이 더 적으면 첫 답으로 되돌아간다 |
| 2026-08-27 | (계측) `subjects_requested`·`subjects_answered` | (없음) → 원장 칸 둘 | — | `parse`는 **온 것 중 버린 것**만 세고 **안 온 것**은 안 셌다. 요청과 응답 수를 함께 남겨야 "넷 중 하나만"이 SQL로 보인다 |
| 2026-08-27 | `MAX_TOOL_ROUNDS` | 3 → 5 | 56건 중 `tool_rounds=3`이 12건(21%). 같은 실행들의 tool call은 19·13으로 상한 32의 절반도 안 씀 — **묶고 있던 것은 호출 수가 아니라 왕복이었다.** 실행 시간은 3왕복에 5분 50초·8분 10초(왕복당 약 2분) | 원장 실측. 정확도 우선이라는 사용자 결정. 장전 창(25분)이 5왕복까지 허용한다 |
| 2026-08-27 | `MAX_TOOL_RESULT_CHARS` | 100,000 → 250,000 | 한 실행이 90,571자(상한의 91%). 호출당 평균 4,767자라 호출 상한 32를 다 쓰면 약 152,000자 | 여유 10%면 안전망이 아니라 일상 제약이다. **실질 브레이크를 `MAX_TOOL_CALLS`로 되돌리는 것이 목적** — 문자 상한이 먼저 마르면 어느 툴이 잘리는지가 호출 순서 운이 된다 |
| 2026-08-27 | (계측) `investigation_truncated` | (없음) → 원장 칸 | — | **끊긴 실행이 조용히 답변으로 넘어간다.** `tool_rounds`만으로는 스스로 끝낸 실행과 구분되지 않아 위 두 값을 "얼마나 부족한지" 없이 올렸다. 다음 판단은 이 칸이 쌓인 뒤에 한다 |
| 2026-08-27 | (계측) `prompt_tokens`·`completion_tokens`·`reasoning_tokens` | (없음) → 원장 칸 셋 | — | **비용이 LangSmith 트레이스에만 있었다.** 슬롯별 추이를 보려면 사람이 하나씩 열어야 했고 2026-08-27 비용 조사가 실제로 그렇게 돌았다. 셋을 나눈 이유와 캐시 적중을 안 세는 이유는 [13-llm-ledger.md](13-llm-ledger.md) "토큰 셋" |
...
| 2026-08-26 | `MAX_TOOL_CALLS` | 20 → 32 | **있다.** 장중 전망 실행이 **21번째 호출**에서 상한에 걸렸는데 그때 툴 결과 누적이 **54,555자로 문자 예산(100,000)의 절반뿐**이었다. 전날 문자 상한만 올리고 호출 상한을 20에 둬서 **호출 수가 먼저 마른 것**이다 — 앞 행의 확인 대상 ②(`tool call budget exhausted`가 뜨는가)가 실제로 떴다 | 5절 실측이 현실적 조사를 26호출로 잡아 거기에 여유를 더했다. **다음에 마르는 것은 문자 쪽이다** — 관측된 호출당 평균 2,598자를 32호출로 외삽하면 83,131자로 예산의 83%다(비례 추정이지 실측이 아니다). `MAX_TOOL_RESULT_CHARS`를 지금 당기지 않는다. 다음 장중 실행의 실제 누적치와 `tool result budget exhausted` 출현을 먼저 본다 |
...
### 2026-08-27 — 한 커밋에서 손잡이 둘을 당겼다

1절의 **"한 번에 한 손잡이"를 지키지 않았다.** 사용자 결정이고 이유는 "이 추론은 실제
판단이라 오래 걸리더라도 정확해야 한다"이다. 둘이 같은 병목의 앞뒤라 따로 올리면 왕복만
늘린 판에서 문자 상한이 먼저 말라 효과를 못 본다.

**그래서 이 판의 효과는 둘을 가르지 못한다.** 다음에 어느 쪽을 다시 볼지는
`investigation_truncated` 비율과 `tool_result_chars` 분포가 따로 말해 준다 — 그 둘을
분리해 읽을 수 있게 만든 것이 같은 커밋에 계측을 넣은 이유다.
...
reasoning 토큰도 그대로 뒀다 — **손잡이가 있는지부터 모른다.** `reasoning_effort`를 xAI가
`grok-4.6`에서 받는지 확인 안 했고, 받더라도 이 흐름의 reasoning은 툴 결과를 소화하는 일이라
줄이면 정확도로 값을 치른다.
...
### 2026-08-28 — 처음으로 기준선과 대조했다. 슬롯 넷을 둘로 줄이고 판을 동결한다

**첫 채점 대조다.** 그전까지 Brier는 절대값으로만 봤고 "0.6이면 좋은가"에 답이 없었다.
3-클래스 Brier는 0~2이고 **균등 추측(1/3씩)이 0.667**이다. 그 선을 그으니 그림이 달라졌다.

채점 84건(2026-08-21~28, `thesis_outcome` 전량):

| 구간 | n | 모델 | 균등 추측 | 사후 실현빈도 | 방향 적중 |
| --- | --- | --- | --- | --- | --- |
| 전체 | 84 | **0.668** | 0.667 | 0.585 | 42% |
| `pre_open` | 44 | 0.634 | 0.667 | 0.573 | 50% |
| `intraday_midday` | 12 | 0.595 | 0.667 | 0.569 | 50% |
| `intraday_afternoon` | 12 | 0.719 | 0.667 | 0.486 | 33% |
| `intraday_morning` | 4 | 0.721 | 0.667 | 0.625 | 25% |
| `pre_close` | 12 | 0.798 | 0.667 | 0.569 | 17% |

**전체는 균등 추측과 구분되지 않고, "최근 실현 빈도대로 찍기"보다 나쁘다.** 적중 42%인데
이 표본에서 "항상 상승"이 54%다. 사후 실현빈도는 그 집합의 결과를 미리 안 상태의 값이라
모델에 불리한 기준이지만, 균등 0.667은 아무 것도 모르는 기준이고 거기서 이미 진다.

**캘리브레이션이 깨져 있다.** `prob_flat` 평균 0.31 vs 실제 flat 13%. 프롬프트가 관측 상태로
실제 기준선(`flat_base_rate`)을 주는데도 모델이 그 두 배 넘게 쓴다. 원인 문장을 찾았다 —
"장중이면 남은 시간이 짧을수록 실제 `flat` 빈도는 아래 기준선보다 높다"가 **실측과 반대다**
(장중 창의 실제 flat 12~25%, 하루 창 13%).

**당긴 것 둘.**

1. **예측 슬롯 넷 → 둘.** `pre_open`(08:35)·`intraday_midday`(12:35)만 남긴다. 뺀 둘은 균등
   추측보다 나빴고 비용의 3분의 1이었다. LLM 실행이 하루 여섯에서 넷으로 준다.
2. **판 12 — `prob_flat` 캘리브레이션.** 위 문장을 지우고, 기준선의 **두 배**를 상한으로
   못박았다. 넘기려면 그날에 한정된 근거가 `claims`에 있어야 한다.

1절의 "한 번에 한 손잡이"를 형식적으로는 어긴다. **슬롯 축소는 품질 손잡이가 아니라 구성
변경이므로 평가를 슬롯별로 하면 섞이지 않는다** — 남은 두 슬롯의 판 11 성적이 이미 위 표에
있고, 판 12 성적을 같은 슬롯끼리 견준다. 전체 평균은 판 11과 비교하지 않는다.

**`prob_up`/`prob_down` 편향(평균 0.32 vs 실제 54%)은 손대지 않았다.** 8일 상승장의 잔상이라
"up을 올려라"는 지난 8일의 방향을 오늘로 미는 것이고 프롬프트가 이미 금지한다.

**확률을 코드에서 보정하지 않는다.** 저장 전에 값을 비틀면 모델이 부르지 않은 숫자가
저장되고 채점이 그것을 모델의 판단으로 센다. 손잡이는 프롬프트 하나다.

#### 판 12는 최소 2주(영업일 10일) 동결한다

지금까지 8일에 판이 여덟(1·2·3·5·6·7·9·11)이었고 판당 n은 4~24였다. **그 상태로는 어떤
변경의 효과도 잴 수 없다** — 이 표를 만들 수 있었던 것도 판을 넘어 전량을 합쳤기 때문이다.

동결 기간에는 프롬프트 문장을 고치지 않는다. 버그는 코드로 고치고, 문장을 고쳐야 하면
그때부터 2주를 다시 센다. 2주 뒤 슬롯당 n은 약 80(대상 4 × 10일 × 슬롯 2)이다.

재측정(운영 DB 읽기 전용):

```sql
select t.run_slot, count(*) n,
       round(avg(o.brier_score), 3) avg_brier,
       round(avg(t.prob_flat), 3) avg_prob_flat,
       round(avg(case when o.actual_outcome = 'flat' then 1.0 else 0 end), 3) actual_flat
from thesis_outcome o join thesis t on t.id = o.thesis_id
where o.brier_score is not null and t.prompt_version = '12'
group by 1 order by 1;
```

**판정 기준.** `avg_prob_flat`이 `actual_flat`에 붙었는데도 슬롯별 Brier가 0.667 아래로 안
내려가면, 프롬프트로 고칠 수 있는 문제가 아니라는 뜻이다. 그때의 선택지는 채점 임계를 창
길이별로 나누는 것(`FLAT_THRESHOLD_PCT`)과 **예측을 접고 기록·설명으로 전환하는 것**이다.
