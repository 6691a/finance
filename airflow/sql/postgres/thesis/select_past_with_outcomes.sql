...
--
-- **`post_nxt_close`도 2026-08-31부터 해설을 받는다**(`19-nxt-narration.md`). 채점은 여전히
-- 없어 `outcomes`의 채점 칸은 비고 해설·판정만 찬다 — `post_close`와 같은 모양이다.
-- 그날 하루는 채점도 해설도 없어 배열 자체가 비어 있었고, 그것이 이 슬롯을 해설 루프에
-- 넣은 이유다(장전이 읽는 슬롯 셋 중 하나만 검증이 없었다).
--
-- 원본은 `thesis.state.PRECEDENT_SLOTS`다. **`NARRATED_SLOTS`와 값이 같아졌지만 상수는
-- 둘로 남는다** — 뜻이 둘이라 다시 갈릴 수 있다(`18-nxt-precedent.md` 2.1절).
--
