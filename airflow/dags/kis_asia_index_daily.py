"""KIS 아시아 지수 확정 일봉 수집 DAG.

니케이225·상해종합·항셍·대만가권의 확정 일봉을 200달력일 창으로 받아 `index_daily`에
`provider = 'kis'`로 쌓는다. 수집기는 미국 일봉(`kis_overseas_index_daily`)과 같은
`KisOverseasIndexDailyCollector`다. 설계는 `docs/collection/kis-overseas-index-close.md` §13이다.

## 왜 Yahoo 일봉을 두고 또 받나

Yahoo 아시아 지수 일봉은 **시가·고가·저가가 틀리고 하루 늦다**(2026-09-04 실측: 최근 66거래일에서
대만가권은 시가가 고가나 저가와 같은 날이 32일, 항셍 12일. 미국 SOX는 2일. 그리고 09-04 오전에
아시아 일봉은 09-02까지만 있었다). 종가는 KIS와 소수점까지 같다. 분봉을 KIS로 받으면서 일봉을
Yahoo로 두면 같은 지수의 두 그레인이 다른 기준을 갖게 된다. Yahoo 행은 지우지 않는다 — 읽는 쪽이
`(provider, symbol)`로 걸어 kis를 본다.

## 기준 날짜

**기대 거래일은 이 run의 `data_interval_end`의 KST 날짜다.** 네 시장의 거래일은 KST 날짜와
같다(도쿄는 같은 시각, 나머지는 한 시간 뒤). 벽시계를 쓰지 않는 이유는 미국 일봉 DAG과 같다 —
DAG을 켤 때 Airflow가 만드는 직전 인터벌 run이 그 인터벌 대신 지금 시각으로 날짜를 잡는다.

거래일은 KIS가 준 `stck_bsop_date`를 그대로 쓴다. 그 시장의 현지 날짜다.

## 휴장

달력이 없다. 한 시장이 쉰 날은 그 날짜 행이 안 올 뿐이고 구간 안의 다른 거래일은 온다.
`index_daily/upsert.sql`이 `(provider, symbol, business_date)`로 멱등 갱신하므로 매일 같은 창을
다시 받아도 안전하다. 구간 전체가 비면(`KisPayloadError`) 그 지수는 실패다.

## params

| 이름 | 기본값 | 뜻 |
| --- | --- | --- |
| `end_date` | `null` | 구간의 끝(YYYY-MM-DD). 비우면 이 run의 KST 날짜 |
| `start_date` | `null` | 구간의 시작(YYYY-MM-DD). 비우면 `end_date`에서 200달력일 앞 |

## 이력 백필

```bash
airflow dags trigger kis_asia_index_daily \\
  --conf '{"start_date": "2016-08-01", "end_date": "2026-09-04"}'
```

KIS가 2016년까지 준다(2026-09-04 실측). 한 응답이 100행이라 200달력일 창은 두 장이고, 가장
오래된 날짜의 전날로 종료일을 옮겨 걷는다. 판단은 `KisOverseasIndexDailyCollector.fetch`가 한다.

## 실패와 재시도

- **한 지수가 실패해도 다른 지수는 저장한다.** 심볼 하나가 트랜잭션 하나다.
- **하루 한 번 도는 확정 수집이라 하나라도 실패하면 죽인다.** 그날 값을 다시 집는 실행이 없다.
  `kis_overseas_index_daily`와 같은 판단이다.
...
from modules.collectors.market.kis_overseas_index import AsiaIndex
...
# KST 평일 18:00 = UTC 09:00. 항셍(KST 17:00 마감, 정산 17:08, 지연 15분)이 가장 늦다.
SCHEDULE = "0 18 * * 1-5"
...
def _session_date() -> date:
    """이 run이 기대하는 아시아 거래일 — run 시각의 KST 날짜. `datetime.now`를 쓰지 않는다."""
...
def requested_end_date(session_date: date, params: dict[str, Any]) -> date:
    """이 run이 구간의 끝으로 쓸 날짜. 비우면 run의 KST 날짜다."""
...
def requested_start_date(end_date: date, params: dict[str, Any]) -> date:
    """이 run이 구간의 시작으로 쓸 날짜. 비우면 200달력일 앞이다. 끝보다 뒤인 시작은 막는다."""
...
@dag(
    dag_id="kis_asia_index_daily",
    dag_display_name="📅 아시아 지수 확정 일봉 (KIS)",
    description="니케이·상해·항셍·대만가권의 확정 일봉을 최근 200달력일 창으로 받아 index_daily에 저장한다.",
    schedule=SCHEDULE,
    ...
    params={
        END_DATE_PARAM: Param(
            ...
            type=["null", "string"],
            title="구간의 끝",
            description="YYYY-MM-DD. 비우면 이 run의 KST 날짜. 이 날짜를 끝으로 최근 200달력일을 받는다.",
        ),
    ...
    doc_md=__doc__,
    tags=["kis", "market", "daily", "asia", "index"],
)
def kis_asia_index_daily():
    @task(task_display_name="아시아 지수 일봉 수집·저장")
    def collect() -> int:
        context = get_current_context()
        ...
        if failures:
            raise AirflowFailException(f"Asian index daily collection failed for: {'; '.join(failures)}")
        return stored

    collect()


kis_asia_index_daily = kis_asia_index_daily()
