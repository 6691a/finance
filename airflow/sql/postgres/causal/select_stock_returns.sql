-- 국내 종목의 실현 등락. 대상 주 안의 변화와 그 뒤 T+1·T+5다.
--
...
--
-- 반환 단위는 percent다(`market_causal_path.return_unit`).
--
-- 국내 종목 일봉의 원본은 `stock_daily`가 아니라 여기다 — 수급과 한 테이블에 있고
-- KIS가 **KRX 정규장** 기준으로 준다(NXT와 시간외는 안 들어 있다).
WITH bars AS (
    SELECT stock_code AS code, business_date, close_price AS close
