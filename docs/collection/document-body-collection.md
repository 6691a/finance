...
1. **첨부 파일 텍스트 변환.** PDF 구현은
   [PDF 파싱·BM25 색인](../analysis/pdf-parsing-bm25.md)을 따른다. HWP는 별도다.
   **운영 Airflow 이미지에 PyMuPDF가 먼저 들어가야 한다.** 스캔·이미지 글자는 로컬 OCR 대신
   선별한 영역만 외부 분석한다. 컬럼은 실제 파서와 읽는 쪽을 붙일 때 `document_attachment`에
   더한다.
2. **키워드 검색은 `pg_search`(BM25)다. `tsvector`가 아니다.** 운영 DB가 ParadeDB이고
   ...
   **검색은 여기서 시작하고 4는 조건이 관측될 때만 켠다**(2026-08-31 사용자 결정).
3. **소비자 붙이기.** market-thesis의 `search_documents` 툴, `apps/api`의
   `/api/documents/search`. **검색 호출을 질의·필터·결과 수·0건 여부와 함께 남긴다** —
   그 로그가 4를 켤지 판정한다.
4. **`document_chunk`와 임베딩 — 조건이 관측되면.** 순서를 2와 바꿨다(2026-08-31). 둘 다
   인프라가 이미 있어(`pg_search` 0.25.2, `pgvector` 0.8.4, 2026-08-30 확인) 비용이 갈리는
   자리는 설치가 아니라 **유지비**다. BM25는 안 쓰면 인덱스 디스크만 먹지만 임베딩은 안 쓰는
   동안에도 새 문서를 매시간 청킹·임베딩해야 최신이고, 모델이 바뀌면 전량을 다시 만든다.
   지금 소비자는 하루 수십 회를 넘지 않을 것으로 본다.
   **켜는 조건 셋과 그때의 모양은 [PDF 파싱 문서](../analysis/pdf-parsing-bm25.md) 12절이
   갖는다.** 모양 자체는 상위 설계 6.2에 그대로 있다 — 청크 2,000자·overlap 200, HNSW.
   미루는 비용은 0이다. 원문을 이미 보존하므로 켤 때 재수집이 없다.
5. **하이브리드 순위.** 4를 켠 뒤에 본다. 그때도 전량 사전 임베딩이 아니라 **BM25 상위
   50~100건을 재순위**하는 형태를 먼저 본다 — 모델이 바뀌어도 다시 만들 것이 없다.
   상위 설계 6.4 ③의 회색지대 판정(코사인 0.80~0.95만 LLM에게 묻는다)도 여기서 함께 선다.
...
- **2026-08-30** 초안. 사용자 요구는 넷이었다 — 본문 저장, 크기 제한 없음, 평가는 제목·요약
  ...
  임베딩과 함께 향후 목록에 적는다.
- **2026-08-31** 14절 향후 목록의 순서를 바꿨다. 임베딩(옛 2)이 BM25(옛 3) 뒤로 갔고,
  소비자 붙이기가 그 사이에 들어왔다. 근거는 유지비다 — 둘 다 인프라가 이미 깔려 있어
  설치 비용이 0이고, BM25는 안 쓰면 놀지만 임베딩은 안 쓰는 동안에도 매시간 돌아야 한다.
  켜는 조건은 [pdf-parsing-bm25.md](../analysis/pdf-parsing-bm25.md) 12절에 적었다.
