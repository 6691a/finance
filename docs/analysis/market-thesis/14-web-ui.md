...
런타임 프런트 의존성은 넷이다(20단계가 `uplot`을 더해 다섯이 됐다).
...
react-router-dom
cytoscape
uplot          # 17단계(2026-08-27). 분봉 53만 행을 표로만 둘 수 없어 들였다
```
...
- ~~chart~~·icon·CSS framework — **chart는 15단계에서 뒤집혔다**(2026-08-27).
  ...
  [20-collection-browser.md](20-collection-browser.md)가 다루는 분봉 53만 행은 표로만 두면
