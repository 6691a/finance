...
from modules.collectors.document.dart import DartPayloadError, Disclosure, FilingEntity
...
_ENTITIES = (FilingEntity(stock_code="005930", name="삼성전자", filing_entity_id="00126380", sector="반도체"),)
...
def test_extraction_failure_is_raised_not_folded_into_not_a_target():
    ...
    with pytest.raises(DartPayloadError):
        dart_disclosure_intraday._extract(_FailingCollector(), _disclosure(), _ENTITIES)


def test_a_report_we_do_not_read_is_still_none():
    """실패가 아니라 대상이 아닌 것은 그대로 `None`이다."""
    assert dart_disclosure_intraday._extract(_FailingCollector(), _disclosure("최대주주변경"), _ENTITIES) is None
