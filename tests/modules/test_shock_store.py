...
from modules.shock.domain import MIN_DOCUMENT_SCORE, PEER_SPECS
...
def test_documents_after_filters_by_score_not_just_order():
    """**정렬만으로는 안 걸러진다.**

    후보가 상한보다 적으면 `LIMIT`이 아무것도 안 자르고 창 안 전부가 모델에 간다.
    `recent_news`가 그 결함을 맞았고(2026-09-04) 이 쿼리도 같은 모양이다.
    """
    connection = FakeConnection([])

    ShockStore(connection).documents_after(
        event_at=DETECTED,
    ...
    statement, parameters = connection.cursors[0].statements[0]
    assert "value_score >= %(min_score)s" in statement
    assert parameters["min_score"] == MIN_DOCUMENT_SCORE
