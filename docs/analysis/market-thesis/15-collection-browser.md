...
- 날짜: 2026-08-27
- 상태: **전 판 구현 완료**(2026-08-27). **저장소의 테이블 49개가 전부 화면까지 닿는다**
  (구현 전 26/49). 마이그레이션이 없어 코드 배포만으로 뜨지만 **프런트가 이미지 안에
  굽히므로 `just build-api`가 먼저다.**
- 산출물: `apps/api/`의 리소스 여섯(`quote`·`indicator`·`document`·`positioning`·
  `event`·`collection`)과 라우트 스물여덟, `frontend/`의 화면 아홉과 `ChartView`·
  `DataTable`·`DatasetBrowser`, API 테스트 42·화면 테스트 45,
