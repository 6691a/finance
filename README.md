...
flowchart LR
    ...
    subgraph AF["Airflow — DAG 48"]
        COL["수집 DAG 35<br/>분봉·일봉·수급·공시·실적·매크로·문서"]
...
| 시각(KST) | 무엇이 도는가 |
| --- | --- |
| 07:00 | 거래일 캘린더(KIS·NYSE) |
| 07:30 · 07:35 | 미국 지수 마감 분봉과 확정 일봉(KIS), Yahoo 일봉 |
| 07:30 ~ 08:50 | 매크로·금리 수집 — FRED, ECOS, 분데스방크, 일본 재무성, BoE, ECB, 국내 수급·포지셔닝 |
| 07:00 ~ 20:50 | 2분마다 DART 공시, 10분마다 공시 알림 |
| 08:00 | **미국장 브리핑** — 밤사이 지수·선물·원자재·금리, 전일 국내 복기. 전 영업일에 포착된 급변이 있으면 **원인 분석** |
...
| 18:00 ~ 18:40 | 아시아·국내 지수와 선물의 확정 일봉(KIS), 투자자별 매매동향 확정, 기술 신호 검출·채점 |
| 19:00 | **코스피 장후 관찰** — 오늘 전망을 채점하고, 무엇이 코스피를 움직였는지를 요인별 엣지로 그래프에 쌓고, 메모를 정리한다 |
| 20:05 | 종목 1분봉 확정본 수집(KRX·NXT 분리) |
| 20:00 · 20:15 | 문서 브리핑과 국내장 브리핑의 NXT 마감본 |

## 코스피 전망 파이프라인

이 프로젝트에서 가장 공들인 부분입니다. LangGraph로 만들었고, 아래 규칙 위에서 돌아갑니다.

**한 번 갈아엎었고 옛 코드는 지웠습니다**(2026-09-03). 먼저 만든 시장 추론은 대상 넷 · 툴 열다섯 · 확률 셋 · 지평 넷으로
...
| `airflow/modules/` | DAG이 쓰는 공유 코드. `collectors/`·`briefing/`·`kospi/`·`technical/`·`expectation/`·`shock/` |
