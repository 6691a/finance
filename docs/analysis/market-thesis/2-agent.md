...
2026-08-25에 20으로 올렸고, **실측에 맞춰 2026-08-26에 32가 됐다.** 세어야 할 것은 툴 개수가
아니라 **대상 수 × 대상별 툴 수**다. 툴을 더 열면 이 값도 같이 본다([TUNING.md](TUNING.md) 5절).
...
| 툴 | 반환 | SQL |
| --- | --- | --- |
| `past_theses(subject_code, n)` | 그 대상의 지난 장전 예측·장후 리뷰(`run_slot`)와 지평별 채점·해설. 슬롯마다 `n`건씩이다. 장전은 같은 조회 `PREFETCHED_PAST_THESES`건을 프롬프트에 **미리 싣고** `thesis_precedent`에 남긴다(5-followup.md 5절). 툴은 더 보고 싶을 때의 길이고 툴로 본 것은 기록되지 않는다 | `thesis/select_past_with_outcomes.sql` |
...
- 프롬프트 규칙: 입력·툴 결과에 없는 사실·숫자 금지, 투자 조언·매수 매도 권유 금지,
  "예측이 아니라 가설이며 채점은 시스템이 한다"를 명시. 숫자 표기는 `llm.NUMBER_STYLE`
  한 벌을 쓴다(천 단위 쉼표, 단위 명시, 툴이 준 단위를 바꾸지 않기). 세 확률이 1에 가깝게 합쳐지도록
...
- **`## 확률` 절이 확률의 뜻을 정의한다**(`PROMPT_VERSION` 5, 2026-08-25). 셋을 넣는다:
  ...
  **2026-08-26에 관측 상태의 `flat_base_rate`로 옮겼다** — 그 비율이 연도별로 단조 감소해
  ...
  확률이 0.32~0.44에 갇혔다. Brier 평균 0.643/0.722로 균등확률 baseline 0.667과 구분되지
  않았다. 같은 판에서 뺀 문구 둘: "확신이 없으면 확률을 고르게 두면 된다"(균등을 정답으로
  만든다)와 "횡보 이유에는 … '방향 정보 없음'을 근거로 써도 된다"(`flat`을 불확실성으로
  읽게 만든 문장이다). **base rate는 매 실행 계산하지 않는다** — 6%인지 7%인지가 판단을
  바꾸지 않고, `FLAT_THRESHOLD_PCT`를 고칠 때 함께 다시 재면 된다.
