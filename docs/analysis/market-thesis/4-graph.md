...
## 8. LLM이 Cypher를 쓰는 조회 층 (2026-08-30)

**설계는 [17-graph-query.md](17-graph-query.md)로 옮겼다.** 조회 층은 배포 단위가 따로라
단계 문서를 하나 받았다 — 가드·흐름·툴·소비자·프로토타입 계획이 거기 있다.

**이 절에는 투영이 만든 제약만 남긴다.** §2·§7.8이 발견한 것이고, 조회를 만드는 쪽이
설계를 다시 하지 말아야 하는 사실들이다.

**미구현이다.** 패키지(`langchain-neo4j`)만 넣어 뒀고 import하는 코드는 없다. 17단계의
프로토타입이 착수 게이트다.
