...
# 6: 둘이 같은 날 들어와 **한 판이다**(2026-08-26, 둘 다 운영에 나가기 전이라 판 2와 같은
#    취급이다). ① 산문의 숫자 표기 규칙(`llm.NUMBER_STYLE`) — 천 단위 쉼표와 단위를 요구한다.
...
#
# **3이던 때 다섯 중 하나가 상한에서 잘렸다**(2026-08-27 실측: `tool_rounds` 분포가 2에 44건,
# 3에 12건). 같은 실행들의 tool call은 19·13으로 `MAX_TOOL_CALLS`(32)의 절반도 안 썼다 —
# 묶고 있던 것은 호출 수가 아니라 왕복이었다. 이 추론은 **오래 걸려도 정확해야 하는 쪽**이라
# 왕복을 늘린다(사용자 결정, 2026-08-27).
#
# **5인 이유는 장전 창이다.** 실측 실행이 3왕복에 5분 50초·8분 10초였다(왕복당 약 2분).
# 5왕복이면 12~14분이고, 장전 전망은 08:35에 시작해 09:00 개장 전에 닿아야 한다.
# 7이면 20분이라 그 창에 붙는다. `BUILD_TIMEOUT`(30분)보다 먼저 개장이 온다.
#
# **잘렸는지는 원장이 센다** — `thesis_llm_run.investigation_truncated`. 그 값이 다시
# 쌓이면 여기를 또 올릴지 판단한다. 값 없이 올리면 다음에도 같은 자리에서 같은 질문을 한다.
MAX_TOOL_ROUNDS = 5
...
#
# **현실적 조사 실측이 26호출이다**(`TUNING.md` 5절). 20이던 때 장중 슬롯이 21번째 호출에서
...
# 24,000에서).
#
# **100,000이던 때 한 실행이 90,571자까지 찼다**(2026-08-27 원장 실측, 3왕복 19호출). 여유가
# 10%면 안전망이 아니라 일상 제약이다. 왕복을 5로 늘리면 이 수도 따라 늘어난다 — 호출당
# 평균 4,767자였으므로 `MAX_TOOL_CALLS`(32)를 다 쓰면 약 152,000자다.
#
# 250,000은 그 호출 상한을 다 써도 60% 남는 크기다. **실질 브레이크를 다시 `MAX_TOOL_CALLS`로
# 되돌리는 것이 이 값의 목적이다** — 문자 상한이 먼저 마르면 어느 툴이 잘리는지가 호출 순서
# 운이 된다(40,000이던 때 실제로 그랬다).
...
# 이유 셋이 **전부** 이보다 짧으면 그 subject를 버린다. 확률·스키마는 멀쩡한데 문장만
# 자리표시자인 응답이 실제로 나왔다(2026-08-27 `intraday_midday`, 셋 다 `dummy`). 스키마는
# 그것을 막지 못하고 Slack까지 그대로 나간다. 한 방향이라도 문장이 있으면 통과시킨다 —
# 모델이 결론 방향만 길게 쓰는 것은 정상이다.
# **한국어 한 문장이 짧다.** "재료가 상쇄됐다"가 8자다 — 자리표시자(`dummy`·`n/a`·`없음`)만
# 걸러지도록 낮게 둔다. 문장의 질을 재는 값이 아니다.
