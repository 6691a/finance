...
> - 한계: 120영업일에 방향 적중이 61.4% 이하이고 폭 적중이 60% 미만이면 전망을 접고 관찰만 남긴다. 요인 15개 중 7개는 첫날 관측이 0이었는데, 지금 구조로는 AI가 안 본 것인지 시장이 안 움직인 것인지 가를 수 없다 — 다음 판부터 코드가 15개를 다 주어 관측 0은 곧 결함이다.

- 날짜: 2026-09-04, 2026-09-06(판 7·8·관찰 판 2·3 예고)
- 상태: **동결 중.** 운영 기동은 2026-09-03. 2026-09-04에 판 5·6으로 올렸고(아래 §0의 기록)
  20영업일 시계는 거기서 다시 시작한다. 판 7·관찰 판 2(§8.10)와 판 8·관찰 판 3(출력 형식을
  YAML에서 응답 모델 description으로 옮김. 문장의 뜻은 그대로)은 코드만 끝났고 동결이 끝나는
  날 함께 배포한다.
...
| (배포 대기) | 6 → 7, 관찰 1 → 2 | 코드는 2026-09-06에 끝났고 **판 6 동결이 끝나는 날 배포한다.** 장후 관찰이 요인을 고르지 않는다 — 코드가 15개 값을 표로 주고 모델이 줄마다 `same`/`inverse`/`none`을 답한다. 전망 문장은 그대로지만 관계 표에 `none`이 실려 판을 올린다. [kospi-forecast.md](kospi-forecast.md) §8.10. 시계는 배포 다음 영업일부터 센다 |
