...
- **새 툴** — 2026-08-21에 일곱을 열어 11개, 2026-08-22에 `analyst_opinions`를 열어 12개가
  됐다([2-agent.md](2-agent.md)). 남은 후보는 `earnings_fact`(지금 6행뿐).
  **금리는 퍼센트 변화가 아니라 bp 차이로 준다** —
  `BASIS_POINT_KINDS`와 `BASIS_POINT_INDICATOR_KINDS`가 그 규칙을 안다. 웹 검색은 출처를
  통제할 수 없어 별도 결정 전까지 안 넣는다.
- **툴 그룹별 서브 에이전트** — 툴이 11개(지금은 12개)가 되면서 나온 이야기다. **지금은 만들지 않는다.**
