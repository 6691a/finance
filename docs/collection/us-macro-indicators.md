...
`NQ=F`가 곧 E-mini 나스닥100 연속 선물이다. 지금 1분봉과 일봉을 둘 다 받고 있고 일봉은 2016년부터 2,515행이 있다.
...
- **전용 화면** — 이 값의 소비자는 화면이 아니라 리포트 쪽 계산이다. 다섯 계열로 따로
  화면을 만들 값어치가 없다. `/indicators`가 종류별로 이미 그린다.
