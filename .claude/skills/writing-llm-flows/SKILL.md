...
실제 flat 13%다(옛 시장 추론 튜닝 기록 2026-08-28, 문서는 지워졌다). 프롬프트가 실제 기준선을 주는데도 두 배 넘게
...
- 근거 0건 / 조사 절단 / 밴드가 중심값만큼 넓음 — 이 셋은 Slack·API에서 정상 답과
  **구별되게** 낸다.
- **저장소가 이미 이 패턴을 쓴다.** 옛 시장 추론의 NXT 선례 검토는 애프터마켓 방향 지속이 56%라
  "**라벨 없이 싣지 않는다**"고 정했다(그 문서는 지워졌고 규칙만 남았다). 같은 규칙을 **자기 출력에도** 적용한다.
...
- 기준선은 상수로 두고 비교는 부르는 쪽이 한다 — 같은 숫자를 SQL과 파이썬에 두 벌로 두지
  않는다. `kospi-evaluation.md` 1절의 기준선(매일 상승 61.4%)이 그 자리다.
- 임계와 조회문을 함께 두는 형태: `kospi-evaluation.md` 4~6절 — 판정마다 복사해 돌리는 SQL이 붙어 있다.
...
- **한 커밋에 손잡이 하나.** 둘을 같이 당기면 효과를 못 가른다. 어쩔 수 없이 둘을 당겼으면
  **그 사실을 문서에 적는다**(`kospi-evaluation.md`의 판 이력 표가 그 자리다).
...
- `kospi-evaluation.md` 6절이 그 형태다 — "120영업일에 방향 적중이 61.4% 이하이면 … **전망을 접고
  관찰만 남긴다**"까지 미리 적어 뒀다.
...
- **모델이 자기 프롬프트를 고치는 층도, 해설에서 규칙을 뽑아 쌓는 층도 만들지 않는다**
  (옛 시장 추론 후속 검토의 결론 — 규칙이 틀리면 예측을 조용히 망친다. 그 문서는 지워졌다).
