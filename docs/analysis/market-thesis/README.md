...
| 4 | [4-graph.md](4-graph.md) | `airflow/modules/graph/projection.py`, `market_causal_weekly`의 `sync_graph` 태스크와 `sync_only` Param, 투영 SQL 셋, requirements, `tests/modules/test_graph.py`. **투영 대상은 `thesis`가 아니라 인과 그래프다**(2026-08-30) | `market-causal-graph.md` | 없음 |
