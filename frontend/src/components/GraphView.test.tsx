...
it("instance를 하나만 만들고 unmount에서 destroy 한다", () => {
  const view = render(
    <GraphView elements={elementsOf(GRAPH)} selected={null} onSelect={() => {}} />,
  );
...
it("elements가 바뀌어도 instance를 새로 만들지 않는다", () => {
  ...
  view.rerender(
    <GraphView elements={elementsOf({ ...GRAPH, edges: [] })} selected={null} onSelect={() => {}} />,
  );
...
it("중심 노드를 root로 한 breadthfirst를 쓴다", () => {
  render(<GraphView elements={elementsOf(GRAPH)} selected={null} onSelect={() => {}} />);
  ...
  expect(core.getElementById).toHaveBeenCalledWith("thesis:1");
  expect(options?.roots).toBeDefined();
});

it("선택은 React state로 올라온다", () => {
  render(<GraphView elements={elementsOf(GRAPH)} selected="thesis:1" onSelect={() => {}} />);
