...
- 날짜: 2026-09-04, 2026-09-06(판 7·8·관찰 판 2·3 배포)
- 상태: **동결 중.** 운영 기동은 2026-09-03. 2026-09-06에 판 7·관찰 판 2(§8.10)와 같은 날
  판 8·관찰 판 3(출력 형식을 YAML에서 응답 모델 description으로 옮김. 문장의 뜻은 그대로)을
  배포했고(아래 §0의 기록) 20영업일 시계는 2026-09-07 장전부터 다시 센다.
...
| 날짜 | 판 | 왜 |
| --- | --- | --- |
| 2026-09-04 | 4 → 5 | 장중 슬롯이 장전 상방 재료를 "갭 소진"으로 접어 말없이 뺐다(장전 14건 → 장중 8건, 이어받은 것 1건). 지시문에 "가격이 움직였다고 재료가 끝난 것은 아니다"를 넣고 앞 슬롯 이유에 요인 코드를 실었다. [kospi-forecast.md](kospi-forecast.md) §8.8. 시계는 다음 영업일 2026-09-07 장전부터 센다 |
| 2026-09-04 | 5 → 6 | `recent_news`에 가치 점수 하한 4를 뒀다. 문장은 그대로고 모델이 보는 기사 묶음이 바뀌어 판만 올렸다. [kospi-forecast.md](kospi-forecast.md) §8.9 |
| 2026-09-06 | 6 → 7, 관찰 1 → 2 | **판 6 동결을 끝까지 기다리지 않았다**(사용자 결정 — 커버리지 구멍이 매 거래일 관측을 잃고 있었다). 장후 관찰이 요인을 고르지 않는다 — 코드가 15개 값을 표로 주고 모델이 줄마다 `same`/`inverse`/`none`을 답한다. 전망 문장은 그대로지만 관계 표에 `none`이 실려 판을 올린다. [kospi-forecast.md](kospi-forecast.md) §8.10. 시계는 2026-09-07 장전부터 센다 |
...
- **관측 커버리지.** 요인 15개 중 몇 개가 실제로 관측되나. 2026-09-03 기준으로 8개만 관측이
  ...
  둘을 지금 구조로는 가를 수 없다 — 모델이 안 조회한 요인은 기록 자체가 없다. 다음 판에서
  코드가 15개 값을 다 주고 모델이 줄마다 `same`/`inverse`/`none`으로 답하게 바꾼다
  ([kospi-forecast.md](kospi-forecast.md) §8.10). 그 뒤에는 관측 0 요인이 곧 결함이다.
