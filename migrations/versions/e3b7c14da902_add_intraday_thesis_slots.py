"""add intraday thesis slots

Revision ID: e3b7c14da902
Revises: c9f4b2e70a18
...
**`downgrade_default()`는 값 집합을 되돌리기만 한다.** 그 시점에 장중 슬롯 행이 남아
있으면 CHECK 생성이 실패하고 **그게 맞다** — 되돌리기가 데이터를 조용히 지우지 않는다.
지울지는 사람이 정한다. `d7a2f4e91c68`과 같은 형태다.

**컬럼 타입은 안 건드린다.** `run_slot`이 String(20)이고 가장 긴 새 값
`intraday_afternoon`이 18자라 그대로 들어간다.
"""
...
revision: str = "e3b7c14da902"
down_revision: str | Sequence[str] | None = "c9f4b2e70a18"
...
# 주석이 값을 나열하므로 함께 다시 찍는다(`d7a2f4e91c68`과 같은 형태).
SLOTS_WITH_INTRADAY = (
    "pre_open",
    "intraday_morning",
    "intraday_midday",
...
SLOTS_BEFORE_INTRADAY = ("pre_open", "post_close", "post_nxt_close")

SLOT_COMMENT_WITH_INTRADAY = (
...
def upgrade_default() -> None:
    _set_slot_axis(SLOTS_WITH_INTRADAY, SLOT_COMMENT_WITH_INTRADAY, AS_OF_COMMENT_WITH_INTRADAY)


def downgrade_default() -> None:
    _set_slot_axis(SLOTS_BEFORE_INTRADAY, SLOT_COMMENT_BEFORE_INTRADAY, AS_OF_COMMENT_BEFORE_INTRADAY)
