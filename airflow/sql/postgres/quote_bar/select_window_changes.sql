...
--
-- 변화를 퍼센트로 만들지 않는다. 금리(`rate`)는 4.65→4.70이 `+1.08%`가 아니라 `+5bp`라
-- 퍼센트로 주면 모델이 급등으로 읽는다(`briefing/market.py`의 `QUOTED_KINDS`와 같은 이유).
-- 종가 둘과 `kind`를 주고 표기는 `modules/thesis.py`가 정한다.
--
