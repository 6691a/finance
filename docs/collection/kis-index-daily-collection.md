...
> 작성 기준: 2026-08-27
>
> 상태: **운영 백필까지 끝났다(2026-08-28).** 커밋 `5a19d9f`(KOSPI200)·`de5e752`·`d146e5f`(국내선물)·
> `6781480`·`d3ac7d7`(미국지수). 리비전 `f8a2c6d9e104`는 운영에 반영됐다
>
> 2026-08-28 읽기 전용 실측: `KOSPI200` 2,460행(2016-08-16~2026-08-27, KOSPI·KOSDAQ와 같은 구간),
> `KOSPI200_FUT`·`KOSDAQ150_FUT` 각 172행(2025-12-12~2026-08-27), `SP500`·`NASDAQ` 각 139행
> (2026-02-09~). **선물이 2025-12-12에서 시작하는 것은 결함이 아니다** — KIS가 만기된 월물을
> 얼마 뒤 지워서 그보다 과거는 받을 방법이 없다(4.4.2절)
>
...
2. **만기된 계약도 과거 날짜로 조회된다.** `A01606`이 만기일 20260611까지 69행을 줬다. 다만
   **영원히는 아니다** — 만기가 한참 지난 월물은 목록에서 지워진다(4.4.2절). 백필 하한이 있다.
...
한 자리다(`A0{자릿수}{year % 10}{MM}`). 10년을 넘기면 `A01609`가 2016년 9월물과 겹친다. **그런데
그 충돌은 실무에서 드러나지 않는다** — 4.4.2절의 보관 한계가 훨씬 먼저 와서 겹치는 상대인 10년
전 계약이 이미 없다. 형식을 넓힐 이유가 지금은 없다.

### 4.4.2 만기물 보관 한계와 백필 하한

2026-08-15부터 백필을 시도했다가 첫 200달력일 창이 통째로 비어 실패했다(2026-08-28). 원인을 찾아
월물 코드를 하나씩 물어본 결과가 다음이다. 모두 그 월물의 자기 창(직전 만기 다음날~만기일)을
요청했다.

| 월물 | 만기 | 만기 후 경과 | 행 |
| --- | --- | --- | --- |
| `A01609`·`A06609` | 2026-09-10 | 미만기 | 54 |
| `A01606`·`A06606` | 2026-06-11 | 2.5개월 | 61 |
| `A01603`·`A06603` | 2026-03-12 | 5.5개월 | 58 |
| `A01512`·`A06512` | 2025-12-11 | 8.6개월 | **0** |
| `A06509` | 2025-09-11 | 11.6개월 | **0** |

사실 셋.

1. **지워진 월물은 조용한 0건이다.** `rt_cd=0`, `msg1="정상처리 되었습니다"`에 `output2`가 빈
   리스트다. HTTP 상태로도 본문 코드로도 "없는 계약"과 "값 없는 구간"이 갈리지 않는다.
2. **창을 넓혀도 안 온다.** `A01512`에 20240101~20251231을 줘도 0행이다. 잘림이나 창 문제가 아니다.
3. **살아 있는 것은 최근 분기물 셋이다.** 그러면 `A01609` 만기(2026-09-10) 즈음에 `A01603`이
   밀려난다고 봐야 한다. 실제 삭제 규칙은 KIS가 밝히지 않으므로 이 표는 관측이지 계약이 아니다.

그래서 **백필 하한은 그때 살아 있는 가장 오래된 월물의 창 첫날**이고, 2026-08-28 기준
`2025-12-12`다(`A01603` 창의 첫날). 이 값은 만기가 지날 때마다 앞으로 밀린다.

하한보다 이른 `start_date`는 조용히 비는 것이 아니라 **실패한다.** `KisFutureDailyCollector.fetch`가
한 200달력일 창에서 한 봉도 못 받으면 `KisPayloadError`를 올리기 때문이다. 창 안의 개별 월물이
비는 것은 정상(주말·휴장)이라 통과시키고, 창 전체가 비는 것만 실패로 본다. 그 판정이 이 사고를
0행 저장 대신 명시적 실패로 드러냈다.

**이력은 받을 수 있을 때 받는다.** DAG가 매일 도는 한 시계열은 앞으로 이어지지만 하한보다 과거는
KIS에서 다시 받을 방법이 없다. 국내선물 확정 일봉의 이력은 구조적으로 9개월 남짓이 상한이고,
지수 일봉의 10년(2016~)과는 애초에 맞출 수 없다.
...
airflow dags trigger kis_future_daily \
  --conf '{"start_date":"2025-12-12","end_date":"2026-08-27"}'

airflow dags trigger kis_overseas_index_daily \
  --conf '{"start_date":"2016-08-15","end_date":"2026-08-27"}'
```

**선물의 시작일은 4.4.2절의 백필 하한이고 고정값이 아니다.** 만기가 지날 때마다 앞으로 밀리므로
백필 전에 그때 살아 있는 가장 오래된 월물을 확인한다. 하한보다 이른 날짜는 태스크를 죽인다.

사용자가 계약 코드를 직접 넣는 파라미터는 만들지 않는다. 논리 시계열의 롤 규칙을 우회할 통로가 된다.
...
1. ~~`KOSPI200`을 기존 국내지수 일봉 순회에 포함한다.~~ **끝났다**(`5a19d9f`,
   `dags/kis_index_daily.py:213`이 `DomesticIndex`를 순회한다).
2. ~~프로브~~ **2026-08-27에 끝났다.** 결과는 4.4·6.3절이다.
3. ~~`index_future_daily.contract_code` 마이그레이션·모델·upsert.~~ **끝났다**
   (리비전 `f8a2c6d9e104`, `apps/models/market/series.py:264`).
4. ~~국내선물 일봉 수집기와 DAG.~~ **끝났다**(`de5e752`·`d146e5f` —
   `collectors/market/kis_future_daily.py`, `dags/kis_future_daily.py`).
5. ~~미국지수 일봉 수집기와 DAG.~~ **끝났다**(`6781480`·`d3ac7d7` —
   `collectors/market/kis_overseas_index_daily.py`, `dags/kis_overseas_index_daily.py`).
6. ~~전체 테스트·정적 검사·오프라인 마이그레이션 SQL.~~ **끝났다.**
7. ~~운영 시험과 백필.~~ **끝났다**(2026-08-28). `KOSPI200`은 2016-08-16까지, 국내선물 둘은
   보관 하한인 2025-12-12까지 채웠다(4.4.2절). 11절 SQL이 그것을 재는 자리다.
8. ~~`graphify update .`~~ **끝났다.**
...
2. `KOSPI200`과 `KOSPI200_FUT` 거래일이 **겹치는 구간에서** 같은 KRX 개장일을 덮음. 구간 자체는
   선물이 짧다(4.4.2절) — 선물 시작일 이전까지 요구하면 이 검사가 늘 실패한다.
