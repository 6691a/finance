...
| [market-causal-graph.md](analysis/market-causal-graph.md) | 주간 사후 인과 그래프. 사건 또는 대상 → 경로 체인 → 대상을 누적해 다중 홉을 만든다 | §1~§8·§11 구현 완료(프롬프트 판 10). **조회 API와 화면은 17단계가 갖는다** |
| [market-thesis/](analysis/market-thesis/README.md) | 시장 추론 기록. 단계마다 문서 하나 | 4(그래프 DB)와 16을 뺀 전부 구현 완료. 12·14·17은 함께 배포한다 — 그 README가 원본 |
