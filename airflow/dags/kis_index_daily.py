...
응답 헤더 `tr_cont`가 오면 연속조회로, 헤더 없이 가득 찬 응답이 오면(확정 수급 API의 행태)
날짜 창을 뒤로 옮겨 받는다. 판단은 `fetch_index_daily`가 한다. 마지막 장까지 받고도 남았으면
그 심볼은 저장하지 않고 실패한다 — 잘린 구간은 지표 계산 창에 구멍을 남긴다.
