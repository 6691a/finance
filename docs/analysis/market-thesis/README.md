...
- 상태: 4단계(그래프 DB)를 뺀 전부 구현 완료이고 **리비전은 운영에
  ...
  **장중 슬롯은 2026-08-28에 넷에서 `intraday_midday` 하나로 줄었다**([TUNING.md](TUNING.md) 6절).
  12·14·20단계는 마이그레이션이 없고 **함께 배포한다** — 화면이 12단계의 클라이언트다.
  배포는 `just build-api` 뒤 `just deploy-api`이고, 프런트 산출물이 이미지 안에 있어
  순서가 뒤바뀌면 옛 화면이 그대로 뜬다. 16단계는 실측이 끝났고 구현만 남았다.
  ...
  **18단계는 2절(장전이 어젯밤 애프터마켓을 본다)까지 구현했고 3절(프리마켓)은 설계만이다**
  (2026-08-31). 리비전이 없어 운영 반영은 배포뿐이다.
  20단계가 **저장소의 테이블을 전부 화면에 올렸다.** 남은 확인은 5절
  **19단계(애프터마켓 해설)도 구현했다**(2026-08-31). 리비전이 없고 상수 둘이라 되돌리는
  값이 싸서 18단계의 5영업일 관측을 기다리지 않았다(사용자 결정).
...
| 17 | [17-graph-query.md](17-graph-query.md) | `market_causal_direction`과 수기 리비전, `airflow/modules/graph/query.py`(고정 Cypher 둘), `causal/direction.py`의 호출–교정 그래프, `prompts/causal_direction.yaml`, `market_causal_weekly`의 `summarize_direction` 태스크, `ObservedState.causal_direction`과 나이 상한, `ThesisEvidenceKind.CAUSAL_PATH`, 프롬프트 `PROMPT_VERSION` 16 | 4 (그리고 `market-causal-graph.md`) | 있음(엇갈린 방향의 종합만 — 세기와 경로는 코드가 만든다) |
...
| 20 | [20-collection-browser.md](20-collection-browser.md) | 수집 원자료 조회 API와 화면 여덟. **저장소의 테이블이 전부 화면까지 닿는다**(구현 전 26/49). uPlot 차트·인과 그래프 조회·대시보드를 들이고 Sentry를 opt-in으로 바꿨다 | 12, 14, 4, 17 | 없음(조회만) |
...
- **20은 14가 남긴 빈자리를 메운다.** Grafana를 걷어내며 사라진 원자료 화면을 우리 계약
  위에 다시 짓는다. **차트 라이브러리 하나(uPlot)를 들이면서 14단계의 "chart 안 넣는다"가
  뒤집혔다** — 분봉 53만 행은 표로만 두면 아무도 못 읽는다
  ([20-collection-browser.md](20-collection-browser.md) 0.3절). 번호가 15에서 20으로 세 번 밀린
  것은 main이 그 사이 15~19를 먼저 쓴 탓이다(2026-08-30·08-31 병합).
- **15는 11이 남긴 자리를 채운다.** 11이 크기 두 칸을 만들면서 축(어느 가격 대비인가)을
  ...
  같은데 ± 오차 값도 추가해줘").
- **18은 7을 5의 루프가 아니라 장전의 재료로 잇는다.** 7이 만든 애프터마켓 리뷰를 읽는
  소비자가 Slack뿐이었고, 다음날 장전은 `past_theses`가 `NARRATED_SLOTS`로 걸러 그것을 못
  봤다. 리뷰 본문(해석)과 NXT 마감가(숫자)를 둘 다 넘기되, 실측(2026-08-31)에서 애프터
  방향이 다음날 정규장으로 이어지는 것이 56%라 **프롬프트가 그 성격을 라벨로 단다.** 해설
  대상 목록은 그대로 두고 되돌아보는 목록만 하나 늘린다. 채점 축은 15가 정한 대로
  정규장이다([18-nxt-precedent.md](18-nxt-precedent.md) 1절).
- **19는 18이 만든 소비자에게 검증을 붙인다.** 18이 애프터마켓 리뷰를 장전 프롬프트로
  이었지만 그 행의 `outcomes`는 채점도 해설도 없어 **영영 빈 배열**이다. 7단계가 해설에서
  뺀 이유는 값어치 판단이 아니라 당시의 하드코딩 결함이었고 그것은 2026-08-23에 풀렸다
  (7단계 9절 취소선). 그래서 남은 질문이 "값어치가 있나" 하나이고, `post_close`에 해설을
  붙인 논리가 그대로 적용된다. **채점은 계속 안 붙인다** — 답을 아는 자리에서 낸 확률이다
  ([19-nxt-narration.md](19-nxt-narration.md) 1절).
