...
2026-08-31에 붙었다**(`19-nxt-narration.md`) — "정규장이 닫힌 뒤 이 재료가 나와서 애프터가
이렇게 움직였다"는 인과 주장이라 며칠 뒤 보도로 검증할 값어치가 있고, 18단계가 이 리뷰를
다음날 장전의 재료로 이으면서 그 판정을 읽는 소비자도 생겼다.

**그 해설을 이 모듈이 만들지 않는다.** 해설은 다음 T+1·3·5의 장후 리뷰
(`market_thesis_review`)가 붙인다. 여기는 슬롯 목록(`thesis.state.NARRATED_SLOTS`)에
들어 있을 뿐이다.
"""
