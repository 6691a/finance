...
> 작성 기준: 2026-08-27
>
> 상태: **코드 구현 완료(2026-08-27).** 커밋 `5a19d9f`(KOSPI200)·`de5e752`·`d146e5f`(국내선물)·
> `6781480`·`d3ac7d7`(미국지수). 리비전 `f8a2c6d9e104`는 운영에 반영됐다
>
> **다만 운영에 값이 들어오는 것은 미국지수뿐이다**(2026-08-28 읽기 전용 실측) —
> `index_daily`의 `kis/SP500`·`kis/NASDAQ`이 2026-02-09부터 139행씩이고
> `KOSPI200`·`KOSPI200_FUT`·`KOSDAQ150_FUT`는 **0행**이다. 10절 7번(운영 시험·백필)이 남았다
>
...
7. **남았다.** 운영은 국내지수 하루치 → 미국지수 하루치 → 선물 하루치 순으로 시험하고,
   마지막에 백필한다. 2026-08-28 실측에서 미국지수만 들어와 있고 `KOSPI200`·국내선물 둘은
   0행이다 — 11절 SQL이 그것을 재는 자리다.
